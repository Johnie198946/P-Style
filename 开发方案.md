...
- **23.2 Part1 Prompt（基础洞察，结合复刻可行性摘要）**
  ...
  - Part1 JSON 要求：
    - `professional_evaluation`：对应 Step 1 的结构化摘要；
    - `analysis_meta`：包含 `conversion_feasibility`、`parameter_interpretation`（对温度/色调等参数的含义解释）、`notes`；
    - `composition`：包含 `advanced_sections` 七段（与第 24 节构图字段表一致）；
    - `workflow_draft`：为 Part2 提供结构化的草案步骤；
