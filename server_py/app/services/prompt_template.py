...
PART2_TEMPLATE = """
{
  ...
  "lightroom_workflow": {
    "simulated_histogram": {
      "description": "中文描述直方图形态（如：直方图整体大幅向右移动（高调），峰值集中在亮部区域，暗部极少且不触底，呈现明快通透的形态）",
      "rgb_values": { "r": 120, "g": 120, "b": 120 },
      "histogram_data": {
        "r": [0, 0, 0, 1, 2, 5, 8, 12, 18, 25, ...256个值，对应亮度级别0-255的红色通道像素数量，归一化到0-100],
        "g": [0, 0, 0, 1, 2, 5, 8, 12, 18, 25, ...256个值，对应亮度级别0-255的绿色通道像素数量，归一化到0-100],
        "b": [0, 0, 0, 1, 2, 5, 8, 12, 18, 25, ...256个值，对应亮度级别0-255的蓝色通道像素数量，归一化到0-100],
        "l": [0, 0, 0, 1, 2, 5, 8, 12, 18, 25, ...256个值，对应亮度级别0-255的亮度通道像素数量，归一化到0-100]
      },
      "stats_grid_description": "中文说明：统计网格显示直方图的三个关键指标：黑色色阶（阴影区域像素占比）、曝光（平均亮度值）、白色色阶（高光区域像素占比）。这些数值帮助理解画面的整体影调分布。",
      "palette_strip_description": "中文说明：调色板条显示从图像中提取的主要颜色，反映画面的色彩倾向和色调分布。每个色块代表图像中的一个主要色彩区域，用于快速识别画面的色彩特征。"
    },
    "basic_panel": {
...
### 🎨 TONE CURVES REQUIREMENTS (色调曲线要求 - 后期领域规范)

**🚨 CRITICAL: 曲线必须符合后期领域的图像规范**

1. **曲线点格式要求**：
   - 所有曲线点必须使用对象格式：`{{"x": 0, "y": 0}}`，其中 x 和 y 都是 0-255 的整数
   - **必须包含起点和终点**：
     - 起点：第一个点的 x 坐标必须是 0（例如：`{{"x": 0, "y": 30}}` 表示黑点被提升到 30）
     - 终点：最后一个点的 x 坐标必须是 255（例如：`{{"x": 255, "y": 255}}` 表示高光保持）
     - **重要**：起点不一定是 (0, 0)，终点不一定是 (255, 255)。例如，Fade Black 效果时起点是 (0, 30)，表示黑点被提升
   - 中间控制点：根据曲线形状添加 3-5 个中间点（例如：`{{"x": 64, "y": 80}}`, `{{"x": 128, "y": 160}}`, `{{"x": 192, "y": 220}}`）

2. **曲线类型要求**：
   - **RGB 曲线**：必须输出完整的曲线点数组（至少 3 个点，包括起点和终点）
   - **单通道曲线**（Red/Green/Blue）：
     - 如果该通道需要调整，输出完整的曲线点数组（至少 3 个点，包括起点和终点）
     - 如果该通道不需要调整，输出默认的直线：`[{{"x": 0, "y": 0}}, {{"x": 255, "y": 255}}]`

3. **曲线形状要求**：
   - 曲线必须是一条平滑的曲线，不能是折线
   - 曲线必须与坐标轴相交（起点在 (0,0)，终点在 (255,255)）
   - 根据 `explanation` 中的描述，合理设置中间控制点：
     - "Fade Black"（黑点提升）：起点 y 值 > 0（例如：`{{"x": 0, "y": 20}}`）
     - "中间调提升"：中间点 y 值 > x 值（例如：`{{"x": 128, "y": 150}}`）
     - "高光压暗"：终点 y 值 < 255（例如：`{{"x": 255, "y": 240}}`）

4. **【专业后期规范】单通道曲线调整原则**：
   - **红色通道（Red Channel）**：
     - 日系/暖调风格：中间调应该**略提**（y > x），增加暖调（微橙/黄）
     - ❌ **禁止**：中间调压暗（y < x），这会导致画面偏冷，不符合日系风格
     - 示例：`{{"x": 128, "y": 135}}` ✅ 正确（略提，增加暖调）
     - 示例：`{{"x": 128, "y": 120}}` ❌ 错误（压暗，导致偏冷）
   - **绿色通道（Green Channel）**：
     - 通常保持直线（无调整），除非有特殊色彩需求
   - **蓝色通道（Blue Channel）**：
     - 日系/空气感风格：黑点可以略提（y > 0），阴影偏蓝，增加空气感
     - 高光应该**保持或略提**（y ≥ 255），保持清透感
     - ❌ **禁止**：高光压暗（y < 255），这会导致高光偏暖，不符合日系清透感
     - 示例：`{{"x": 255, "y": 255}}` ✅ 正确（保持高光清透）
     - 示例：`{{"x": 255, "y": 250}}` ❌ 错误（压暗高光，导致偏暖）
   - **色彩分离协调性**：
     - RGB 曲线整体提亮时，单通道曲线应该**协调配合**
     - 红色通道中间调应该与 RGB 曲线方向一致（提升）
     - 蓝色通道高光应该与 RGB 曲线方向一致（保持或提升）

4. **示例**：
   ```json
   "tone_curves": {{
     "explanation": "日系空气感曲线：RGB曲线左下角黑点上提（Fade Black），中间调大幅拱起以提亮整体，高光部分保持平滑。",
     "points_rgb": [
       {{"x": 0, "y": 20}},    // 起点：黑点上提
       {{"x": 64, "y": 80}},   // 中间调：大幅提升
       {{"x": 128, "y": 140}}, // 中间调：继续提升
       {{"x": 192, "y": 220}}, // 高光：平滑过渡
       {{"x": 255, "y": 255}}  // 终点：保持高光
     ],
     "points_red": [
       {{"x": 0, "y": 0}},
       {{"x": 128, "y": 135}},  // 【专业规范】红色通道：中间调略提，增加暖调（符合日系风格）
       {{"x": 255, "y": 255}}
     ],
     "points_green": [
       {{"x": 0, "y": 0}},
       {{"x": 255, "y": 255}}  // 绿色通道：通常保持直线，无调整
     ],
     "points_blue": [
       {{"x": 0, "y": 10}},    // 蓝色通道：暗部上提（阴影偏蓝，增加空气感）
       {{"x": 128, "y": 135}}, // 蓝色通道：中间调略提
       {{"x": 255, "y": 255}}  // 【专业规范】蓝色通道：高光保持，不压暗（保持清透感，符合日系风格）
     ]
   }}
   ```

**⚠️ 重要：如果某个通道不需要调整，也必须输出起点和终点，不能输出空数组！**
...
1. **master_style_recap**: 确认识别到的流派（例如："日系冬日极简 / 高调雪景风格 (Japanese Winter Minimalist)"）
2. **style_summary_recap**: 简要回顾 Phase 1 的核心指导思想（例如："基于参考图的[Winter Minimalist]与[Soft Contrast]，核心在于极柔和的影调过渡与空气感。"）
3. **key_adjustment_strategy**: 总结三大关键调整动作（例如："1. 极哑光黑位(Matte Black)以模拟胶片质感；2. 负向清晰度与去朦胧以制造雨天/空气感；3. 青粉色调分离，统一画面情绪。"）
...
1. **description**: 中文描述直方图形态（例如："直方图整体向左移动（暗调），峰值集中在中间偏左区域..."）
2. **rgb_values**: RGB 平均值对象，包含 `r`、`g`、`b` 三个整数值（0-255）
3. **histogram_data**: **必需字段**，包含完整的直方图数据对象，包含以下四个数组：
   - `r`: 红色通道的直方图数据数组（至少 20 个数值，对应亮度级别 0-255 的分布）
   - `g`: 绿色通道的直方图数据数组（至少 20 个数值）
   - `b`: 蓝色通道的直方图数据数组（至少 20 个数值）
   - `l`: 亮度通道的直方图数据数组（至少 20 个数值）
   
   **重要**：`histogram_data` 不能为空数组！必须根据 `description` 中描述的直方图形态，生成对应的数值数组。
   例如：如果描述是"直方图整体向左移动（暗调）"，则 `histogram_data` 中的数组应该在左侧（低亮度区域）有较高的值，右侧（高亮度区域）值较低。
   数值范围建议在 0-100 之间，表示每个亮度级别的像素分布强度。

4. **stats_grid_description**: **必需字段**，中文说明统计网格（Stats Grid）的含义和作用。
   **要求**：必须详细说明四个单元格的具体含义，以及它们与上方直方图的关系。
   **格式**：从左到右依次说明：
   - 第一个单元格（黑色色阶）：说明该数值表示阴影区域（0-85 亮度级别）的像素占比百分比，数值越大表示暗部细节越多。该数值对应直方图左侧区域的像素分布密度。
   - 第二个单元格（曝光）：说明该数值表示画面的平均亮度值（0-255），数值越大表示画面整体越亮。该数值对应直方图峰值位置的水平坐标。
   - 第三个单元格（白色色阶）：说明该数值表示高光区域（171-255 亮度级别）的像素占比百分比，数值越大表示高光细节越多。该数值对应直方图右侧区域的像素分布密度。
   - 第四个单元格（Clipping）：说明是否出现高光或阴影溢出，NONE 表示无溢出，画面细节完整。
   **示例**："统计网格（Stats Grid）位于直方图下方，从左到右四个单元格分别显示：①黑色色阶（24%）：表示阴影区域（0-85 亮度级别）的像素占比，对应直方图左侧区域的像素密度，数值越大说明暗部细节越丰富；②曝光（127）：表示画面平均亮度值（0-255），对应直方图峰值位置，数值越大画面整体越亮；③白色色阶（25%）：表示高光区域（171-255 亮度级别）的像素占比，对应直方图右侧区域的像素密度，数值越大说明高光细节越丰富；④Clipping（NONE）：表示无高光或阴影溢出，画面细节完整。这些数值与上方直方图的形态直接对应，帮助理解画面的整体影调分布特征。"

5. **palette_strip_description**: **必需字段**，中文说明 HSL 混色器调整面板的含义和作用。
   **要求**：必须详细说明面板显示的内容、与 Lightroom HSL 混色器的关系，以及调整值的含义。
   **格式**：说明面板显示的内容（哪些颜色有调整，H/S/L 各代表什么），以及这些调整对画面的影响。
   **示例**："HSL 混色器调整面板位于直方图与统计网格之间，显示需要在 Lightroom HSL 混色器中调整的颜色及其参数值。每个色块代表一个需要调整的颜色通道，显示三个参数：H（色相偏移）、S（饱和度调整）、L（明度调整）。例如：橙色 H-5 S-20 L+10 表示将橙色色相向红色方向偏移 5 度、降低饱和度 20%、提高明度 10%，主要用于优化肤色表现。当前画面中，橙色（肤色核心）需要降低饱和度并提亮，蓝色（天空/阴影）需要减少饱和度以降低视觉干扰，绿色（植被）需要轻微偏黄以营造暖调氛围。这些调整值可直接在 Lightroom HSL 混色器面板中输入使用。"
