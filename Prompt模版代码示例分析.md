...
**⚠️ 问题**：文档中只显示了一个 `simulated_histogram_data`，没有明确说明是否包含 `reference` 和 `user` 两个字段。
...
3. **情况C**：在 `color_depth_analysis` 中分别有两个字段
   ```json
   {
     "color_depth_analysis": {
       "text": "...",
...
├── main_structure (画面主结构分析)
├── subject_weight (主体位置与视觉权重)
│   ├── description (主体位置、占比)
│   └── layers (前景/中景/远景)
├── visual_guidance (线条与方向引导)
│   ├── analysis (线条走向分析)
│   └── path (视觉路径：入口点 -> 停留点 -> 终点)
├── ratios_negative_space (比例与留白)
│   ├── entity_ratio (实体比例，如 "60%")
│   ├── space_ratio (留白比例，如 "40%")
│   └── distribution (留白分布位置)
└── style_class (构图风格归类)
```
...
```
将新结构的5个字段映射到现有的7段：
1. 画面主结构分析 ← main_structure
2. 主体位置与视觉权重 ← subject_weight.description
3. 线条与方向引导 ← visual_guidance.analysis
4. 空间层次与分区 ← subject_weight.layers
5. 比例与留白 ← ratios_negative_space.distribution
6. 视觉平衡与动势 ← visual_guidance.path
7. 构图风格归类与改进建议 ← style_class
```
...
**理由**：
1. 符合新 Prompt 的结构，数据映射简单
2. 每个字段独立显示，信息完整
3. 前端改动可控（只需修改 `CompositionSection.tsx`）

**实施**：
- 修改 `CompositionSection.tsx`，将7段改为5段
- 每个字段对应一个卡片
- `subject_weight` 和 `ratios_negative_space` 可以显示子字段
