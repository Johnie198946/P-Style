...
| | 7. **[组件集成]** 修改 `VisualAnalysisDashboard.tsx`：<br>      a. 添加 `dominantColors` 状态<br>      b. 将 `Vectorscope` 的 `onDominantColorsExtracted` 回调连接到状态<br>      c. 将 `dominantColors` 传递给 `AIAnalysisPanel` |
| | 8. **[API 封装]** 在 `src/src/lib/api.ts` 中添加 `analyze.diagnosis` 方法 |
| **后端** | 1. **[Prompt 模板]** 在 `server_py/app/services/prompt_template.py` 中新增 `get_diagnosis_prompt` 方法：<br>      a. 接收直方图数据和主色调列表<br>      b. 生成专业的诊断 Prompt（包含评分要求、输出格式） |
| | 2. **[Schema 定义]** 在 `server_py/app/schemas/analysis_schemas.py` 中新增：<br>      a. `DiagnosisRequestSchema`：诊断请求 Schema<br>      b. `DiagnosisResponseSchema`：诊断响应 Schema<br>      c. `validate_diagnosis_response`：响应验证函数 |
...
- **前端状态与轮询**：
  ...
  - **超时与防多次点击**：
    - **前端超时设置**：
      - `uploadRequest` 函数默认超时 120 秒（2 分钟），适用于可行性评估和 Part1 分析。
      - 如果 Gemini API 响应时间较长（如 Part1 可能达到 60-70 秒），前端超时时间足够。
      - 如果超时，前端会显示友好的错误提示："请求超时，请稍后重试"。
    - **后端超时设置**：
      - Gemini API 调用超时时间从 `GEMINI_TIMEOUT_MS` 配置读取，默认 120 秒。
...
- **Python 侧（`server_py/app/services/gemini_service.py`）**：
  - 客户端封装：
    - 使用 `google.genai.Client(api_key=GEMINI_API_KEY)` 初始化，在 `GeminiService` 中注入：
      ...
      - `timeout_ms`：从配置 `GEMINI_TIMEOUT_MS` 读取，默认 120 秒（120000 毫秒）。
    - 方法设计：
    ...
    - **超时控制**：
      ...
      - 超时时间从 `GEMINI_TIMEOUT_MS` 配置读取，默认 120 秒。
...
- **23.3 Part2 Prompt（执行参数与工作流，对齐科学仿色约束）**
  ...
  - JSON 部分（严格数值输出）：
    - **新结构**（当前使用）：
      - `phase_1_extraction`：
        - `style_summary_recap`：简要回顾 Phase 1 的核心指导思想（提取自上一阶段的 `style_summary`）
        - `key_adjustment_strategy`：总结本次调色的三大关键点（光影重塑核心、色彩偏移核心、质感核心）
      - `color_science_scheme`：
        ...
        - `hsl_detailed_12_colors`：HSL 12 色详细调整（red、orange、yellow、yellow_green、green、green_cyan、cyan、cyan_blue、blue、blue_purple、purple、magenta），每个颜色包含 `h`、`s`、`l`、`desc`）
          ...
          - **绿色 (Green)**：
            ...
            - Lum: 提升 (+10) -> 树叶透光感
      - `lightroom_workflow`：
        - `simulated_histogram`：预判调色后的直方图形态（`description`、`rgb_values`、`zone_values`）
...
- **Lighting（光影参数） → `results.lighting`**
  - 前端字段（新 Prompt 模版）：
    ...
    - `toneCurves?: {` ⭐ **新增字段**
      ...
      - `points_rgb?: [number, number][]` - RGB 曲线坐标点数组
      - `points_red?: [number, number][]` - 红色通道曲线坐标点数组
      - `points_green?: [number, number][]` - 绿色通道曲线坐标点数组
      - `points_blue?: [number, number][]` - 蓝色通道曲线坐标点数组
      - `}`
    - `texture?: {`
