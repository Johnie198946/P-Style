...
| | 6. **[数据适配]** `dataAdapter.ts`：提取量化分析数据到前端：<br>      a. 从 `backendData.meta.image_analysis` 提取到 `result.lightroom.meta.image_analysis`<br>      b. 从 `sections.color.structured.scene_type` 提取到 `result.lightroom.color.scene_type`<br>      c. 从 `structured.analysis` 和 `phase_1_extraction` 提取分析数据 |
| | 7. **[安全拦截]** `LightroomPanel.tsx`：在 `filterParams` 计算中实现 Safety Clamps：<br>      a. **Auto-Exposure Override**: 如果 `lumaMean < 50`，强制曝光至少 `+1.0 EV`<br>      b. **Skin Tone Lock**: 如果场景包含 "portrait" 或 "人像"，限制 Orange HSL：Hue `±5`，Saturation `>-10`<br>      c. **Dynamic Range Clamp**: Saturation 限制在 `±40`，防止 AI 输出极端值 |
| | 8. **[类型定义]** `types/analysis.ts`：更新 `LightroomData` 接口：<br>      a. 添加 `meta?: { image_analysis?: any }`<br>      b. 添加 `color?: { scene_type?: string }`<br>      c. 添加 `analysis?` 和 `phase_1_extraction?` 字段 |
| | 9. **[渲染]** LR/PS/色彩卡片根据`structuredAnalysis`中的数据自动渲染。 |
| | 10. **[渲染]** 在Figma的“仿色示意图”位置，渲染`structuredAnalysis.preview_image_url`。 |
| | 11. **[渲染]** “仿色后”的`Histogram`组件，传入`structuredAnalysis.preview_image_url`。 |
| **后端** | 1. **[Prompt]** `promptTemplate.js`：创建`getPart2Prompt(part1Context, ...)`函数，内容为你Prompt的Step 3, 5, 6 + V4.0的“工作流执行总结”。 |
| | 2. **[量化分析]** `image_analyzer.py`：在调用 Part2 Prompt 前，使用 OpenCV 计算用户图的量化指标：<br>      a. **`luma_mean` (0-255)**: 全图平均亮度（`<60` 为欠曝，`>200` 为过曝）<br>      b. **`histogram_dist`**: 直方图重心（暗部/中间调/高光占比）<br>      c. **`color_cast`**: 白平衡偏移（B_avg - R_avg，判断冷暖）<br>      d. 调用 `compare_images(ref_bytes, user_bytes)` 获取完整分析数据 |
| | 3. **[数据注入]** `prompt_template.py`：实现 `_build_input_data_injection_section()` 方法：<br>      a. 生成 `[SYSTEM_DETECTED_METRICS]` 段落，插入 Prompt 最前面<br>      b. 格式：`User_Image_Luminance_Mean: 45 -> STATUS: Severely Underexposed`<br>      c. 强制 Gemini 识别曝光/色温问题，而非"拍脑袋"生成参数<br>      d. 在 `get_part2_prompt()` 中调用此方法，注入量化数据 |
| | 4. **[元数据传递]** `analyze.py`：在 `_run_part2_analysis_job()` 中：<br>      a. 将 `image_analysis` 注入到 `structured_result.meta.image_analysis`<br>      b. 前端可通过 `data.meta?.image_analysis?.user` 获取量化数据<br>      c. 用于前端 Safety Clamps（自动曝光补偿、肤色保护等） |
| | 5. **[Prompt]** `promptTemplate.js`：创建`getPart3FlashPrompt(userImage, styleParams)`函数。<br>      *Prompt示例：“你是一个修图师。将这张[用户图]按照以下[LR/PS参数或风格描述]进行处理，生成一张仿色后的示意图。”* |
| | 6. **[路由]** `POST /api/analyze/part2`：<br>      a. （受Auth保护）获取`taskId`。<br>      b. **[异步]** **不使用`await`**，调用一个`async`函数 `runPart2And3Job(taskId)`。<br>      c. **立即返回**`{ status: 'processing' }`。 |
| | 7. **[路由]** `GET /api/analyze/:taskId`：<br>      a. （受Auth保护）查询`analysis_tasks`表。<br>      b. 返回`{ status, structured_result, natural_language_part1, ... }`。 |
| | 8. **[核心Job]** `runPart2And3Job(taskId)`：<br>      a. **量化分析**: 调用 `compare_images()` 获取 `image_analysis`<br>      b. **Part 2**: 调用`geminiService.js` (Part 2 Prompt + 量化数据注入)，获取LR/PS JSON。<br>      c. **验证**: `const validatedP2 = Part2ResponseSchema.parse(geminiJson)`（**Zod在此！**）<br>      d. **元数据注入**: 将 `image_analysis` 添加到 `structured_result.meta.image_analysis`<br>      e. **Part 3**: 调用`geminiService.js` (Gemini Flash，使用Part 3 Prompt)，获取示意图URL/Base64。<br>      f. **[存储]** 将`validatedP2`（LR/PS参数）和`preview_image_url`（示意图）**合并**到`structured_result`中。<br>      g. **[更新]** `UPDATE analysis_tasks SET structured_result = ?, status = 'completed', part2_completed = 1 WHERE id = ?`。 |
| | 9. **[存储]** （可选但推荐）如果示意图是Base64，应先上传到对象存储（如S3/OSS），只在数据库存URL。 |
...
>   * **`image_analysis`**: (新增) OpenCV 量化分析数据，包含 `luma_mean`, `histogram_dist`, `color_cast`
>
> 【任务】
> 你将执行第二阶段的详细参数执行。根据`part1_context`中的草案，为`user_image`生成复刻`reference_image`的详细参数。
>
> **【重要更新 - Implementation Specification】**
>
> Part 2 Prompt 已升级为"开发者级别的实施规范"，采用三层架构：
>
> **第一层：输入端的数据注入 (Input Data Injection)**
>
> - **目的**：解决 Gemini "眼盲"问题，通过代码层面的量化分析，强制 AI 识别曝光/色温问题
> - **实现**：在 Prompt 最前面插入 `[SYSTEM_DETECTED_METRICS]` 段落：
>   ```
...
>   ```
> - **计算指标**：
>   - `luma_mean` (0-255): 全图平均亮度（`<60` 为欠曝，`>200` 为过曝）
>   - `histogram_dist`: 直方图重心（暗部/中间调/高光占比）
>   - `color_cast`: 白平衡偏移（B_avg - R_avg，判断冷暖）
> - **后端实现**：`server_py/app/services/prompt_template.py` → `_build_input_data_injection_section()`
>
> **第二层：Prompt 核心逻辑与参数映射 (Prompt & Mapping)**
>
> - **伪代码级指令集**：使用 Gap Analysis、Anchor Color Locking、Dynamic Range Matching 等认知协议
> - **硬映射表**：
...
> - **JSON 输出模板**：严格执行 `PART2_TEMPLATE` 结构，包含 `phase_1_extraction`、`analysis`、`lightroom_workflow` 等字段
>
> **第三层：微操控制与安全阀 (Safety Protocols)**
>
> - **目的**：在代码层面拦截 AI 输出的极端值，防止毁图参数
> - **前端实现**：`src/components/analysis/LightroomPanel.tsx` → `filterParams` 计算逻辑
> - **安全拦截**：
>   1. **Auto-Exposure Override**: 如果 `lumaMean < 50`，强制曝光至少 `+1.0 EV`
>   2. **Skin Tone Lock**: 如果场景包含 "portrait"，限制 Orange HSL：Hue `±5`，Saturation `>-10`
>   3. **Dynamic Range Clamp**: Saturation 限制在 `±40`，防止 AI 输出 `+100` 等极端值
> - **数据流**：后端 `image_analysis` → `structured_result.meta.image_analysis` → 前端 `data.meta?.image_analysis?.user`
>
> 1.  **自然语言报告 (Part 2)**：
...
#### 16.11 图像量化分析功能（OpenCV 数据驱动参数生成）

**背景**：解决 Gemini "拍脑袋"生成参数的问题，从"凭感觉猜参数"升级为"基于真实图像数据生成精准参数"。

**实现文件**：
- `server_py/app/services/image_analyzer.py`：图像分析服务（核心实现）
- `server_py/app/services/prompt_template.py`：Prompt 模板服务（数据注入）
- `server_py/app/routes/analyze.py`：分析路由（流程集成）

**核心功能**：

1. **图像分析服务 (`ImageAnalyzer` 类)**：
   - `analyze_image(image_data: bytes)`：分析单张图片
     - 直方图分析：黑点 RGB、白点 RGB、峰值位置、平均亮度、阴影/中间调/高光分布、形态描述
     - 色彩分析：平均 RGB/HSV、估算色温、主要颜色（K-Means 聚类）、饱和度水平
     - 区域分析：高光区域、中间调区域、阴影区域、天空检测、植被检测
   - `compare_images(ref_data: bytes, user_data: bytes)`：对比两张图片
     - 计算差值：黑点提升量、曝光调整 EV、色温差值 K、饱和度差值、对比度差值
     - 生成建议：曲线起点 y 值、色温调整方向、饱和度调整方向、特殊提醒（天空/植被）

2. **数据注入到 Prompt**：
   - `PromptTemplate._build_quantitative_data_section(image_analysis)`：将分析结果格式化为 Markdown 段落
   - 注入位置：在 `style_summary_note` 之后，`phase_1_extraction` 说明之前
   - 数据格式：包含参考图分析、用户图分析、差值分析、AI 调整建议

3. **流程集成**：
   - `server_py/app/routes/analyze.py` 的 `_run_part2_analysis_job()` 函数：
     1. 在调用 Gemini 之前先执行图像分析
     2. 将 base64 图片数据转换为二进制
     3. 调用 `compare_images()` 获取量化数据
     4. 将 `image_analysis` 传递给 `get_part2_prompt()`
     5. Prompt 中包含量化数据段落，Gemini 基于真实数据生成参数

**数据流**：
```
用户上传图片
    ↓
OpenCV 分析（image_analyzer.py）
    ↓
生成量化数据（直方图、色彩、区域、差值、建议）
    ↓
注入到 Part2 Prompt（prompt_template.py）
    ↓
Gemini 基于真实数据生成参数
    ↓
更精准的调色参数
...
| 之前（拍脑袋） | 现在（数据驱动） |
|---------------|-----------------|
| "日系风格，曲线起点 y=35" | "参考图黑点亮度=36，曲线起点 y=36" |
| "偏冷，色温 -500K" | "参考图色温 5200K，用户图 5780K，色温 -580K" |
| "降饱和度" | "参考图饱和度 22%，用户图 40%，饱和度 -18%" |
| "天空偏青" | "参考图天空 H=205°，S=15%，建议用蒙版单独调整" |

**技术细节**：
- 使用 OpenCV (`cv2`) 进行图像处理
- 使用 NumPy 进行数值计算
- 黑点/白点计算：取最暗/最亮的 1% 像素的平均值
- 色温估算：基于 RGB 比例简化算法（非精确计算，仅供参考）
- 天空检测：图像上部 1/3 + 蓝色/青色色相 + 中等饱和度 + 较高亮度
- 植被检测：绿色色相 + 中等饱和度
- K-Means 聚类：用于提取主要颜色（默认 5 种）
...
**错误处理**：
- 如果图像分析失败，不会中断 Part2 流程
- `image_analysis` 为 `None` 时，Prompt 中不包含量化数据段落
- Gemini 仍可基于视觉观察生成参数（降级方案）
...
- **23.3 Part2 Prompt（执行参数与工作流，对齐科学仿色约束）**
  ...
  - 输入：
    ...
    - `image_analysis`：**OpenCV 图像量化分析数据（新增，核心输入）**
      - **目的**：解决 Gemini "拍脑袋"生成参数的问题，基于真实图像数据生成精准参数
      - **实现**：`server_py/app/services/image_analyzer.py` 使用 OpenCV 分析参考图和用户图
      - **分析内容**：
        1. **直方图分析**：黑点 RGB、白点 RGB、峰值位置、平均亮度、阴影/中间调/高光分布、形态描述
        2. **色彩分析**：平均 RGB/HSV、估算色温、主要颜色（K-Means 聚类）、饱和度水平
        3. **区域分析**：高光区域、中间调区域、阴影区域、天空检测、植被检测
        4. **差值计算**：黑点提升量、曝光调整 EV、色温差值 K、饱和度差值、对比度差值
        5. **调整建议**：曲线起点 y 值、色温调整方向、饱和度调整方向、特殊提醒（天空/植被）
      - **数据注入**：`prompt_template.py` 的 `PromptTemplate._build_quantitative_data_section()` 方法将分析结果格式化为 Markdown 段落，注入到 Part2 Prompt 中
      - **后端流程**：`server_py/app/routes/analyze.py` 的 `_run_part2_analysis_job()` 函数在调用 Gemini 之前先执行图像分析
      - **数据格式**：`compare_images()` 返回包含 `reference`、`user`、`deltas`、`recommendations` 的字典
      - **Prompt 注入位置**：量化数据段落插入在 `style_summary_note` 之后，`phase_1_extraction` 说明之前
      - **重要性**：这是从"凭感觉猜参数"到"基于数据生成参数"的关键改进，大幅提升参数精准度
    - `part1_context`：Part1 上下文（含 `workflow_draft` 等，可选）
    - `feasibility_result`：可行性评估结果摘要（可选）
  - **处理逻辑（必须严格遵守）**：
    1. **量化分析优先**：**必须基于 OpenCV 图像分析的真实数据生成参数，而非凭感觉猜测**
       - 曲线起点 y 值应接近参考图的黑点亮度值（如参考图黑点亮度=36，曲线起点 y=36）
       - 色温调整应接近计算出的差值（如参考图色温 5200K，用户图 5780K，色温调整 -580K）
       - 饱和度调整应基于差值计算（如参考图饱和度 22%，用户图 40%，饱和度调整 -18%）
       - 曝光调整应基于平均亮度差值（转换为 EV 值）
       - **如果量化数据与 Phase 1 风格指导冲突，优先遵循量化数据，但需在 `reason` 中说明原因**
    2. **继承战略**：生成的每一个参数（如色温、曲线点）都必须有据可依，必须源于 Phase 1 的分析结果或量化分析数据。严禁产生与 Phase 1 矛盾的参数（例如 Phase 1 说要冷调，你却加暖色温）。
    3. **软件分工**：
    ...
    4. **数据精度**：
       ...
       - **量化数据说明**：在 `reason` 字段中应引用量化分析结果，例如："参考图黑点亮度=36，有褪色感，曲线起点提升至 y=36" 或 "参考图色温 5200K，用户图 5780K，色温调整 -580K"
...
## 28. Lightroom 实时预览功能（2024-12）

### 28.1 功能概述

在 Lightroom 面板中实现类似 Photoshop 图层的实时预览功能，用户可以：
1. 点击「关闭预览」按钮展开预览图
2. 查看所有 Lightroom 调整参数应用后的效果
3. 通过图层面板独立查看各个调整的效果（Solo 模式）
...
| 组件 | 文件路径 | 功能 |
|------|----------|------|
| ImageEngine | `src/src/lib/ImageEngine.ts` | WebGL 图像处理引擎 |
| LivePreviewCanvas | `src/components/analysis/LivePreviewCanvas.tsx` | 预览画布组件 |
| LightroomPanel | `src/components/analysis/LightroomPanel.tsx` | 主面板（参数转换） |

#### 28.2.2 ImageEngine 支持的调整

**基础调整（已实现）**：
- ✅ 曝光 (Exposure): `-5.0` ~ `+5.0` EV
- ✅ 对比度 (Contrast): `-100` ~ `+100`
- ✅ 高光 (Highlights): `-100` ~ `+100`
- ✅ 阴影 (Shadows): `-100` ~ `+100`
- ✅ 白色 (Whites): `-100` ~ `+100`
- ✅ 黑色 (Blacks): `-100` ~ `+100`
- ✅ 色温 (Temperature): 负值偏蓝/冷，正值偏橙/暖
- ✅ 色调 (Tint): 负值偏绿，正值偏洋红
- ✅ 饱和度 (Saturation): `-100` ~ `+100`
- ✅ 自然饱和度 (Vibrance): `-100` ~ `+100`

**色彩分级（已实现）**：
- ✅ 阴影色调 (Shadow Tint): 色相 0-360°，饱和度 0-100%
- ✅ 中间调色调 (Midtone Tint): 色相 0-360°，饱和度 0-100%
- ✅ 高光色调 (Highlight Tint): 色相 0-360°，饱和度 0-100%

**曲线（已实现）**：
- ✅ RGB 曲线: 使用 LUT（查找表）+ Catmull-Rom 样条插值

### 28.3 参数转换规则

从 Gemini 输出到 ImageEngine 的参数转换：

```typescript
// LightroomPanel.tsx - filterParams 转换逻辑
const filterParams: FilterParams = {
  // 曝光：Gemini 输出 -100~+100，转换为 -5~+5
  exposure: getParamValue(safeBasicPanel.exposure) / 20,
  
  // 对比度：直接传递，ImageEngine 内部 /100
  contrast: getParamValue(safeBasicPanel.contrast),
  
  // 色温：Gemini 输出绝对值（如 5490K），转换为相对偏移
  // 5500K 是中性点，低于 5500K 偏蓝（负值）
  temperature: ((getParamValue(safeBasicPanel.temp) || 5500) - 5500) / 25,
  
  // 色调：直接传递
  tint: getParamValue(safeBasicPanel.tint),
  
  // 色彩分级
  shadowsHue: data.color_grading?.shadows?.hue || 0,
  shadowsSat: data.color_grading?.shadows?.saturation || 0,
  // ...
  
  // 曲线
  curve: safeData.curve?.points_rgb || safeData.curve?.rgb || undefined,
};
```

### 28.4 WebGL Shader 实现

**片段着色器处理流程**：

1. **白平衡**：
   - 色温：调整 R/B 通道（暖色增加红、减少蓝）
   - 色调：调整 G 通道（洋红减少绿）

2. **曝光**：`rgb *= pow(2.0, exposure)`

3. **对比度**：`rgb = (rgb - 0.5) * (1 + contrast) + 0.5`

4. **高光/阴影/白色/黑色**：使用 `smoothstep` 创建亮度蒙版
...
**问题：预览显示黑色**
- 原因：WebGL shader 编译错误
- 解决：检查浏览器控制台的 "Shader compile error" 日志

**问题：参数效果不明显**
- 原因：参数转换比例不正确
- 解决：检查 `filterParams` 的转换逻辑

---

## 29. Implementation Specification - 开发者级别的实施规范（2025-01-29）
...
**调用位置**: `get_part2_prompt()` 方法中，在 Prompt 最前面插入此段落。

### 29.4 第二层：Prompt 核心逻辑与参数映射
...
#### 29.5.2 Auto-Exposure Override（自动曝光补偿）
...
#### 29.5.4 Dynamic Range Clamp（动态范围压缩）
...
**问题：色温/色调方向相反**
- 原因：shader 中的颜色通道调整方向错误
- 解决：确保色温负值偏蓝、色调正值偏洋红


---

# 第11章 高保真渲染引擎集成方案 (High-Fidelity Rendering Engine)

## ⚠️ 重要更新：渲染引擎实现变更 (2024-12-02 更新)

**当前实现**：采用 **双模式渲染策略** - 优先使用 **RawTherapee CLI**，如果 Docker 不可用则回退到 **Python 渲染引擎**。

**变更历史**：
1. **初始方案**：计划使用 Darktable CLI 进行高保真渲染
2. **2024-12 中期**：由于 XMP 格式复杂（需要二进制编码），切换到纯 Python 引擎
3. **2024-12-01**：用户反馈 Python 渲染不准确，恢复 Darktable CLI 为首选渲染方式
4. **2024-12-02**：发现 Darktable XMP 格式不兼容问题（明文 XML 无法被识别），**完全迁移到 RawTherapee CLI**

**迁移原因**：
- **Darktable XMP 格式问题**：Darktable 4.6+ 要求 XMP 参数必须是 Base64 编码的二进制数据块，外部生成极其困难且不稳定
- **RawTherapee 优势**：使用标准的 INI 格式（PP3 文件），完全支持程序化生成，且画质与 Lightroom/Darktable 同属专业级
- **生产级稳定性**：PP3 格式是文本键值对，版本升级不影响参数读取，符合生产环境要求

**当前策略**：
```
if Docker 可用 && RawTherapee 容器运行:
    使用 RawTherapee CLI 渲染（高保真，PP3 配置文件）
else:
    回退到 Python 渲染引擎（近似效果）
```

**实现位置**：
- `server_py/app/services/hifi_render_service.py`：高保真渲染服务（双模式）
- `server_py/app/services/rawtherapee_pp3_generator.py`：PP3 文件生成器（INI 格式）
- `server_py/app/services/python_render_engine.py`：Python 渲染引擎（备用）
- `server_py/docker/Dockerfile.rawtherapee`：RawTherapee Docker 镜像构建文件

**使用 RawTherapee CLI 的步骤**：
1. 构建 RawTherapee Docker 镜像：`cd server_py/docker && docker-compose build pstyle-rawtherapee`
2. 启动容器：`docker-compose up -d pstyle-rawtherapee`
3. 验证：`docker exec pstyle-rawtherapee rawtherapee-cli -h`

**技术栈**：
- **RawTherapee CLI**（首选）：专业级 RAW 处理软件，PP3 配置文件为文本格式，完全支持程序化生成
- **Python 引擎**（备用）：OpenCV/NumPy 实现的近似效果渲染

**优先级说明**：
- RawTherapee CLI：高保真，效果接近 Lightroom，PP3 格式稳定可靠，需要 Docker
- Python 引擎：近似效果，无需 Docker，作为回退方案

**已完全移除 Darktable**：
- ❌ 已删除 `server_py/app/services/darktable_xmp_generator.py`
- ❌ 已删除 `server_py/docker/Dockerfile.darktable`
- ❌ 已删除 `server_py/docker/render_solo.sh`（Darktable 专用脚本）
- ❌ 已停止并删除 `pstyle-darktable` Docker 容器
...
## 1. 背景与目标（已更新）
*   **现状**：当前预览基于前端 WebGL (`ImageEngine.ts`)，仅能进行近似模拟。对于高级参数（Dehaze, Highlights Recovery, Clarity）与 Lightroom 原生算法存在差异。
*   **原始目标（已废弃）**：引入 **Darktable CLI** 作为后端渲染核心，在 Docker 容器中运行，将 Gemini 生成的 Lightroom 参数转换为 Darktable 处理指令，生成与 LR 极其接近的高保真预览图。
*   **当前实现**：使用 **Python 图像处理引擎**（OpenCV/NumPy）进行高保真渲染，不需要 Docker 容器。详见上方"重要更新"部分。

## 2. 架构设计

采用 **"实时 WebGL 预览 + 异步 CLI 高保真渲染"** 的双层架构。

```mermaid
graph TD
    Client[前端 UI] -->|1. 实时调整 (LR JSON)| WebGL[前端 WebGL 引擎 (低保真/实时)]
    Client -->|2. 请求高清预览 (LR JSON)| Server[Python 后端 API]
    Client -->|3. SOLO 模式请求| Server
    
    subgraph Docker Environment
        Server -->|4. 参数映射 (JSON -> XMP)| Mapper[参数映射服务]
        Mapper -->|5. 生成 Sidecar| XMPFile[.xmp 文件]
        Server -->|6. 调用 CLI| CLI[Darktable CLI 容器]
        CLI -->|7. 读取| RawImg[原图]
        CLI -->|8. 读取| XMPFile
        CLI -->|9. 渲染输出| RenderedImg[高保真 JPG/WebP]
    end
    
    Server -->|10. 返回图片 URL| Client
```
...
| 组件 | 选型 | 原因 |
| :--- | :--- | :--- |
| **渲染核心** | **Darktable CLI** | Linux 下最强 LR 替代品，支持 CLI 批处理。 |
| **源码获取** | **GitHub Clone** | 从 https://github.com/darktable-org/darktable 克隆并编译，确保版本可控。 |
| **环境构建** | **Docker (多阶段构建)** | 隔离 Linux 依赖，编译阶段与运行阶段分离，减小镜像体积。 |
| **交互方式** | **Python `subprocess`** | 后端通过命令行直接调用容器内的 CLI 工具。 |
| **参数载体** | **XMP Sidecar** | Darktable 使用 XMP 记录编辑参数。 |
...
```dockerfile
# ===============================================
# 阶段1：从 GitHub 克隆并编译 Darktable
# 源码地址: https://github.com/darktable-org/darktable
# ===============================================
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    libgtk-3-dev libxml2-dev libexiv2-dev liblcms2-dev \
    liblensfun-dev librsvg2-dev libjpeg-dev libpng-dev \
    libtiff-dev libwebp-dev libopenjp2-7-dev libcurl4-openssl-dev \
    libsqlite3-dev libjson-glib-dev libgphoto2-dev libpugixml-dev \
    libheif-dev ocl-icd-opencl-dev liblua5.4-dev intltool xsltproc

# 克隆 Darktable 源码（锁定版本 release-4.6.0）
WORKDIR /build
RUN git clone --depth 1 --branch release-4.6.0 \
    https://github.com/darktable-org/darktable.git

# 编译 Darktable（仅 CLI 模式）
WORKDIR /build/darktable
RUN mkdir build && cd build && \
    cmake .. -G Ninja \
        -DCMAKE_BUILD_TYPE=Release \
        -DCMAKE_INSTALL_PREFIX=/opt/darktable \
        -DBUILD_USERMANUAL=OFF -DBUILD_CMSTEST=OFF \
        -DUSE_OPENMP=ON -DUSE_OPENCL=ON -DUSE_LUA=ON \
    && ninja && ninja install
...
# 安装运行时依赖
RUN apt-get update && apt-get install -y --no-install-recommends \
    libgtk-3-0 libxml2 libexiv2-27 liblcms2-2 liblensfun1 \
    librsvg2-2 libjpeg62-turbo libpng16-16 libtiff6 libwebp7 \
    libopenjp2-7 libcurl4 libsqlite3-0 libjson-glib-1.0-0 \
    libgphoto2-6 libpugixml1v5 libheif1 liblua5.4-0 \
    ocl-icd-libopencl1 imagemagick exiftool python3

# 从 builder 复制 Darktable
COPY --from=builder /opt/darktable /opt/darktable
ENV PATH="/opt/darktable/bin:$PATH"

WORKDIR /app
COPY render_script.sh render_solo.sh /app/
RUN chmod +x /app/*.sh

HEALTHCHECK --interval=30s --timeout=10s --retries=3 \
    CMD darktable-cli --version || exit 1
```
...
# 构建镜像
cd server_py/docker
docker build -t pstyle-darktable:4.6.0 -f Dockerfile.darktable .
...
**SOLO 模式**与前端 `LightroomPanel.tsx` 的 SOLO 按钮对应，允许用户**单独预览某个参数的效果**，而其他参数保持默认值。
...
### 5.2 SOLO 参数到 Darktable 模块映射

```python
# server_py/docker/render_solo.sh 中的映射逻辑
PARAM_TO_MODULE = {
    # 基本面板
    'exposure': ['exposure'],
    'contrast': ['filmicrgb'],
    'highlights': ['filmicrgb', 'shadhi'],
    'shadows': ['filmicrgb', 'shadhi'],
    'temperature': ['temperature'],
    'tint': ['temperature'],
    
    # 存在感
    'texture': ['localcontrast'],
    'clarity': ['localcontrast'],
    'dehaze': ['hazeremoval'],
    'vibrance': ['colorbalancergb'],
    'saturation': ['colorbalancergb'],
    
    # HSL（统一映射到 colorzones）
    'hsl_red': ['colorzones'],
    'hsl_green': ['colorzones'],
    'hsl_blue': ['colorzones'],
    # ... 其他颜色
    
    # 色彩分级
    'grading_shadows': ['colorbalancergb'],
    'grading_midtones': ['colorbalancergb'],
    'grading_highlights': ['colorbalancergb'],
    
    # 校准
    'calibration': ['channelmixerrgb'],
}
```
...
```typescript
// src/components/analysis/LightroomPanel.tsx

// 当用户点击 SOLO 按钮时，判断是否使用高保真渲染
const handleSoloToggle = useCallback(async (layerKey: string) => {
  ...
  // 如果启用了高保真模式，触发 CLI 渲染
  if (hiFiAvailable && enableHiFiSolo) {
    const response = await hifiRenderService.renderSolo({
      image_path: userImageUrl,
      solo_param: layerKey,
      ...currentFilterParams,
      use_cache: true
    });
...
}, [previewMode, hiFiAvailable, enableHiFiSolo, userImageUrl]);
```

## 6. 参数映射策略 (Mapping Strategy)

| Lightroom 参数 | Darktable 对应模块 (Module) | 映射逻辑 |
| :--- | :--- | :--- |
| **Exposure** | `exposure` | `dt_exposure = lr_exposure` |
| **Temp/Tint** | `temperature` | Temp 需转换 Kelvin 值，Tint 对应绿色通道系数。 |
| **Contrast** | `filmic rgb` | 建议映射到 `filmic rgb` 的 contrast 属性。 |
| **Highlights/Shadows** | `shadows and highlights` | 需校准强度比例（LR±100 ≈ DT±1.0）。 |
| **Dehaze** | `haze removal` | `dt_haze_strength = lr_dehaze * 0.01` |
| **Clarity** | `local contrast` | 映射到局部对比度模块。 |
| **HSL** | `color zones` | 映射到 Darktable 的色区曲线中。 |
| **Color Grading** | `color balance rgb` | 对应 Darktable 的 4 路色彩平衡。 |
| **Calibration** | `channel mixer rgb` | 映射到通道混合器模块。 |
...
```http
POST /api/render/high-fidelity
Content-Type: application/json

{
  "image_path": "user_upload_12345.jpg",
  "basic": { "exposure": "+0.8", "contrast": "+10", ... },
  "hsl": { "green": { "hue": "+15", "saturation": "+20", "luminance": "+10" } },
...
| 操作 | WebGL 预览 | CLI 高保真渲染 |
| :--- | :--- | :--- |
| **拖拽调整参数** | ✅ 实时响应 | ❌ 不触发 |
| **松手后** | ✅ 保持显示 | ⏳ 可选触发（防抖 500ms） |
| **点击 HQ RENDER** | ❌ | ✅ 触发 |
| **SOLO 模式切换** | ✅ 实时切换 | ✅ 可选高保真 |

### 8.2 对比模式

前端提供 WebGL/CLI 对比开关：
- **并排对比**：左侧 WebGL 预览，右侧 CLI 高保真
- **滑块对比**：拖动滑块切换两种渲染结果
...
server_py/
├── docker/
│   ├── Dockerfile.darktable     # 多阶段构建 Dockerfile
│   ├── docker-compose.yml       # 容器编排配置
│   ├── render_script.sh         # 完整渲染脚本
│   └── render_solo.sh           # SOLO 模式渲染脚本
├── app/
│   ├── services/
│   │   ├── darktable_xmp_generator.py  # LR JSON → XMP 转换器
│   │   └── hifi_render_service.py      # 高保真渲染服务
│   └── routes/
│       └── render.py            # 渲染 API 端点

src/
├── src/lib/
│   └── hifiRenderService.ts     # 前端高保真渲染服务
└── components/analysis/
    └── LightroomPanel.tsx       # HQ RENDER 按钮和 SOLO 集成
```
...
```bash
# 1. 构建 Darktable 镜像（耗时约 10-15 分钟）
cd server_py/docker
...
### 10.2 升级 Darktable 版本

```bash
# 修改 Dockerfile.darktable 中的版本号
git clone --branch release-4.8.0 ...
...
1. **首次构建耗时**：使用包管理器安装 Darktable 约需 2-3 分钟（比源码编译快 5 倍）。
2. **GPU 加速**：如果服务器有 NVIDIA GPU，可以启用 OpenCL 加速（需要在 docker-compose.yml 中配置 GPU 透传）。
3. **缓存策略**：基于参数 Hash 的缓存可以显著减少重复渲染，建议定期清理过期缓存。
4. **参数校准**：LR 和 Darktable 的参数范围可能不同，需要实际测试并校准映射系数。
...
#### Docker 环境
- [x] Dockerfile 配置（使用 Debian 包管理器安装 Darktable 4.2.1）
- [x] docker-compose.yml 配置
- [x] 镜像构建成功（454MB）
- [x] 容器运行正常（健康状态：healthy）
- [x] 渲染脚本就绪（`render_script.sh` 和 `render_solo.sh`）

#### 后端服务
- [x] `HiFiRenderService` 服务类实现
- [x] `DarktableXmpGenerator` XMP 生成器
- [x] `/api/render/health` 健康检查端点
- [x] `/api/render/high-fidelity` 完整渲染端点
- [x] `/api/render/solo` SOLO 模式渲染端点
- [x] `/api/render/solo/batch` 批量 SOLO 渲染端点
- [x] 路由注册到 `main.py`

#### 前端服务
- [x] `hifiRenderService.ts` 客户端服务
- [x] API 客户端集成（修复调用方式）
- [x] LightroomPanel 添加 HQ RENDER 按钮
- [x] 渲染状态管理
- [x] 中英文翻译支持
...
- [x] `BUILD.md` 构建说明
- [x] `verify.sh` 验证脚本
- [x] `test_render.sh` 测试脚本
- [x] `INTEGRATION_TEST.md` 集成测试指南
...
| 组件 | 状态 | 版本/详情 |
|------|------|----------|
| **Docker 镜像** | ✅ 已构建 | `pstyle-darktable:4.6.0` (454MB) |
| **Darktable CLI** | ✅ 可用 | 4.2.1 |
| **容器运行** | ✅ 正常 | healthy |
| **后端 API** | ✅ 已实现 | 4 个端点 |
| **前端集成** | ✅ 已完成 | UI 和状态管理 |
| **文档** | ✅ 已完成 | 5 个文档文件 |
...
#### 问题 1：React 组件初始化顺序错误 ✅

**问题描述**：
- **错误信息**：`Uncaught ReferenceError: Cannot access 'filterParams' before initialization`
- **错误位置**：`LightroomPanel.tsx:1908:41`
- **根本原因**：`handleHiFiRender` 函数在 `filterParams` 定义之前，但在 `useCallback` 的依赖数组中引用了 `filterParams`

**修复方案**：
1. 将 `handleHiFiRender` 的定义移到 `filterParams` 的 `useMemo` 定义之后
2. 确保所有使用 `filterParams` 的代码都在其定义之后
3. 添加中文注释说明初始化顺序的重要性

**修复位置**：
- `src/components/analysis/LightroomPanel.tsx` 第 2562 行（修复后位置）

**经验教训**：
- React Hooks 的依赖数组必须引用已定义的变量
- 使用 `useCallback` 和 `useMemo` 时要注意初始化顺序
- 代码审查时应检查依赖关系

#### 问题 2：高保真渲染 API 图片路径错误 ✅

**问题描述**：
- **错误信息**：`原图不存在: /Users/dengzhaoyu/Desktop/TepVis/AI产品/P-Style/server_py/storage/uploads/blob:http:/localhost:3001/5e8accec-8490-4994-aa7c-50a1463361cc`
- **错误位置**：`hifiRenderService.ts:180` 和 `render.py:212`
- **根本原因**：前端传递的 `userImageUrl` 是浏览器生成的 blob URL，不是实际的文件路径。后端尝试在文件系统中查找这个路径，导致失败。

**修复方案**：
1. **后端修改**：
   - 修改 `HiFiRenderRequest`，添加可选的 `task_id` 参数
   - 修改 `render_high_fidelity` 函数，支持从 `task_id` 查询数据库获取实际文件路径
   - 如果 `target_image_url` 是对象存储路径（格式：`uploads/{user_id}/{task_id}/target-{uuid}.jpg`），直接使用
   - 如果是 Base64 data URL，保存为临时文件后使用

2. **前端修改**：
   - 修改 `HiFiRenderRequest` 接口，`image_path` 改为可选，添加 `task_id` 字段
   - 修改 `buildRequestFromFilterParams` 方法，添加 `taskId` 参数
   - 修改 `handleHiFiRender` 函数，优先使用 `taskId`，从 sessionStorage 读取 `current_task_id`

**修复位置**：
- `server_py/app/routes/render.py` - 添加 taskId 支持和数据库查询逻辑
- `src/src/lib/hifiRenderService.ts` - 添加 taskId 参数支持
- `src/components/analysis/LightroomPanel.tsx` - 传递 taskId 到渲染服务

**经验教训**：
- 浏览器 blob URL 不能直接用于后端文件系统操作
- 应该使用数据库存储的实际文件路径或对象存储路径
- 通过 taskId 关联查询是更可靠的方式

#### 问题 3：高保真渲染 API 无法找到图片路径（404 错误）✅

**问题描述**：
- **错误信息**：`无法从任务 {task_id} 获取图片路径`
- **错误位置**：`render.py:251` - 未找到关联的上传记录
- **根本原因**：
  1. `Upload` 表和 `AnalysisTask` 表之间没有关联（`analysis_task_id` 字段为空）
  2. 渲染 API 只从 `Upload` 表查询，没有从 `AnalysisTask` 表获取图片数据
  3. `analyze_part1` 创建任务后没有调用 `link_to_task` 关联上传记录

**修复方案**：
1. **渲染 API 修复** (`server_py/app/routes/render.py`)：
   - 优先从 `Upload` 表查询（通过 `analysis_task_id` 或 `upload.id`）
   - 如果 `Upload` 表找不到，从 `AnalysisTask.target_image_data` 获取 Base64 数据
   - 将 Base64 数据保存为临时文件后使用
   - 添加详细的日志记录和错误信息

2. **分析任务关联修复** (`server_py/app/routes/analyze.py`)：
   - 在 `analyze_part1` 创建任务后，调用 `UploadService.link_to_task` 关联上传记录
   - 确保 `Upload.analysis_task_id` 字段被正确设置
   - 添加异常处理，确保关联失败不影响主流程

**修复位置**：
- `server_py/app/routes/render.py` - 添加从 `AnalysisTask.target_image_data` 获取图片的逻辑
- `server_py/app/routes/analyze.py` - 添加 `link_to_task` 调用

**经验教训**：
- 数据库表之间的关联关系需要明确维护
- 应该提供多种数据获取方式作为备用方案
- 关键操作应该添加详细的日志记录，便于问题定位

#### 问题 4：高保真渲染成功但图片没有变化 ✅

**问题描述**：
- **现象**：渲染请求返回成功，但渲染后的图片与原图没有区别
- **根本原因**：
  1. XMP 文件没有正确传递给 Darktable CLI
  2. `/app/input` 目录是只读挂载（`:ro`），无法复制 XMP 文件
  3. Darktable CLI 命令没有指定 XMP 文件参数

**修复方案**：
1. **XMP 文件复制修复** (`server_py/app/services/hifi_render_service.py`)：
   - 将 XMP 文件复制到 `/app/xmp` 目录（可写），而不是 `/app/input` 目录
   - 使用 `docker cp` 命令将 XMP 文件复制到容器

2. **Darktable CLI 命令修复**：
   - 修改命令格式，使用正确的语法：`darktable-cli input.jpg input.xmp output.jpg`
   - XMP 文件作为第二个参数传递给 Darktable CLI
   - 添加 `--hq true` 参数确保高质量渲染

3. **清理逻辑**：
   - 渲染完成后清理容器内的临时 XMP 文件
   - 确保所有异常情况下都能清理临时文件

**修复位置**：
- `server_py/app/services/hifi_render_service.py` - 修复 XMP 文件复制和 Darktable CLI 命令

**经验教训**：
- Docker 挂载配置需要仔细考虑读写权限
- Darktable CLI 的正确用法需要查阅文档确认
- 关键步骤（如 XMP 文件传递）需要验证是否真正执行

#### 问题 5：高保真渲染无法找到对象存储图片（HTTP URL）✅

**问题描述**：
- **错误信息**：`原图不存在: /Users/dengzhaoyu/Desktop/TepVis/AI产品/P-Style/server_py/storage/uploads/3/4347e5b1-d28e-457c-961c-dee374a76bba/target-ecb844c7-3a7b-41f7-947a-53eeceedd8d3.jpg`
- **根本原因**：
  1. `target_image_url` 是 HTTP URL（`http://localhost:9000/photostyle/uploads/...`），不是本地文件路径
  2. 代码只处理了相对路径（`uploads/...`）和 Base64 data URL，没有处理 HTTP/HTTPS URL
  3. 渲染服务尝试在本地文件系统中查找 HTTP URL，导致失败

**修复方案**：
1. **HTTP URL 处理修复** (`server_py/app/routes/render.py`)：
   - 添加对 HTTP/HTTPS URL 的检测和处理
   - 使用 Python 标准库 `urllib.request.urlopen` 下载图片到本地临时文件
   - 保存到 `storage/uploads/temp_{task_id}_target.jpg`
   - 使用临时文件路径进行渲染

2. **错误处理**：
   - 添加详细的日志记录（下载进度、文件大小等）
   - 添加异常处理（URLError, HTTPError）
   - 确保下载失败时返回清晰的错误信息

**修复位置**：
- `server_py/app/routes/render.py` - 添加 HTTP URL 下载逻辑

**经验教训**：
- 对象存储 URL 和本地文件路径需要区分处理
- 应该支持多种图片来源（本地文件、Base64、HTTP URL）
- 临时文件管理需要确保清理机制

#### 问题 6：HTTP URL 被误判为本地路径 ✅

**问题描述**：
- **错误信息**：`原图不存在: /Users/dengzhaoyu/Desktop/TepVis/AI产品/P-Style/server_py/storage/uploads/3/284f6aac-a10c-4759-a01c-a85f078575b9/target-91c83bae-de3f-4755-b6e7-549179b8cc82.jpg`
- **根本原因**：
  1. 代码先检查了 `/uploads/` 是否在 URL 中，如果是就提取相对路径
  2. 但 HTTP URL（如 `http://localhost:9000/photostyle/uploads/...`）中也包含 `/uploads/`
  3. 导致 HTTP URL 被错误地当作本地路径处理，提取了相对路径 `3/284f6aac-.../target-...jpg`
  4. 这个相对路径在本地文件系统中不存在，导致渲染失败

**修复方案**：
1. **调整判断顺序** (`server_py/app/routes/render.py`)：
   - 先检查是否是 HTTP/HTTPS URL（`startswith("http://")` 或 `startswith("https://")`）
   - 如果是 HTTP URL，直接下载到本地临时文件
   - 然后再检查是否是本地路径（`startswith("uploads/")` 或包含 `/uploads/` 但不是 HTTP URL）
   - 确保 HTTP URL 不会被误判为本地路径

2. **改进本地路径判断**：
   - 在检查 `/uploads/` 时，需要排除 HTTP URL
   - 使用条件：`startswith("uploads/") or ("/uploads/" in url and not url.startswith("http"))`

**修复位置**：
- `server_py/app/routes/render.py` - 调整 URL 类型判断顺序

**经验教训**：
- 条件判断的顺序很重要，应该先检查更具体的条件（HTTP URL），再检查更通用的条件（包含特定字符串）
- 字符串匹配时需要考虑边界情况（如 HTTP URL 中包含 `/uploads/`）
- 代码逻辑应该清晰，避免歧义

#### 问题 7：对象存储图片下载失败（HTTP 403 Forbidden）✅

**问题描述**：
- **错误信息**：`下载图片失败: HTTP Error 403: Forbidden`
- **根本原因**：
  1. 对象存储（MinIO）需要认证才能访问
  2. 直接使用 `urllib` 下载 HTTP URL 时，没有提供认证信息
  3. MinIO 的 bucket 可能配置了访问控制，需要 Access Key 和 Secret Key

**修复方案**：
1. **添加 StorageService 下载方法** (`server_py/app/services/storage_service.py`)：
   - 添加 `download_image` 方法，支持从 MinIO/OSS/本地文件系统下载图片
   - 使用 MinIO 客户端下载（支持认证），而不是直接使用 HTTP 请求
   - 对于本地文件系统，直接读取文件
   - 添加详细的日志记录和错误处理

2. **修改渲染 API** (`server_py/app/routes/render.py`)：
   - 使用 `StorageService.download_image` 下载图片，而不是直接使用 `urllib`
   - 如果 StorageService 下载失败，返回清晰的错误信息
   - 确保下载的图片保存为临时文件后用于渲染

**修复位置**：
- `server_py/app/services/storage_service.py` - 添加 `download_image` 方法
- `server_py/app/routes/render.py` - 使用 StorageService 下载图片

**经验教训**：
- 对象存储访问需要认证，应该使用客户端 SDK 而不是直接 HTTP 请求
- 统一的存储服务接口可以简化代码，提高可维护性
- 错误处理应该提供清晰的错误信息，帮助用户理解问题

#### 问题 7.1：StorageService download_image 方法排查和修复 ✅

**排查过程**：
1. **检查日志**：MinIO 客户端初始化成功，但下载失败
2. **检查 URL 解析逻辑**：发现 bucket_name 可能未在所有情况下初始化
3. **检查代码逻辑**：发现 `_init_local` 方法没有设置 bucket_name
4. **添加详细日志**：添加调试日志追踪下载过程

**发现的问题**：
1. **bucket_name 未初始化**：
   - `_init_local` 方法没有设置 `self.bucket_name`
   - 如果 MinIO 初始化失败，bucket_name 可能未被设置
   - 导致 URL 解析失败：`self.bucket_name in url` 检查失败

2. **缺少详细日志**：
   - download_image 方法缺少详细的调试日志
   - 无法追踪 URL 解析和下载过程

**修复方案**：
1. **修复 bucket_name 初始化** (`server_py/app/services/storage_service.py`)：
   - 在 `__init__` 方法开始时设置 `self.bucket_name`（确保所有情况下都被设置）
   - 在 `_init_local` 方法中也设置 `self.bucket_name`（保持一致性）
   - 确保即使初始化失败，bucket_name 也有默认值

2. **添加详细日志**：
   - 在 download_image 方法中添加详细的调试日志
   - 记录 URL、storage_type、bucket_name、object_key 等信息
   - 记录每个分支的执行情况

3. **改进错误处理**：
   - 在 URL 解析失败时，记录详细的检查条件
   - 帮助快速定位问题

**修复位置**：
- `server_py/app/services/storage_service.py` - 修复 bucket_name 初始化和添加详细日志

**经验教训**：
- 初始化顺序很重要，应该确保所有必要的属性在所有情况下都被设置
- 详细的日志对于调试复杂问题非常重要
- 代码应该考虑所有可能的执行路径
...
1. **API 端点测试**
   - [ ] 测试健康检查端点：`GET /api/render/health`
   - [ ] 测试完整渲染端点：`POST /api/render/high-fidelity`
   - [ ] 测试 SOLO 渲染端点：`POST /api/render/solo`
   - [ ] 验证错误处理和日志记录

2. **参数映射验证**
   - [ ] 测试基本面板参数映射（exposure, contrast, highlights, shadows）
   - [ ] 测试 HSL 参数映射
   - [ ] 测试色彩分级参数映射
   - [ ] 测试校准参数映射
   - [ ] 对比 WebGL 预览和 CLI 渲染结果
...
1. **参数映射校准**
   - [ ] 对比 Lightroom 和 Darktable 的实际渲染效果
   - [ ] 调整参数映射系数
   - [ ] 优化 XMP 生成逻辑
   - [ ] 处理边界情况（极端参数值）
...
1. **参数映射精度**
   - 当前实现为基础版本，LR 和 Darktable 的参数映射可能需要根据实际效果调整
   - 某些高级参数（如 Dehaze、Clarity）的映射可能需要特殊处理

2. **版本差异**
   - 当前使用 Debian 仓库的 Darktable 4.2.1（非最新版本）
   - 如需最新版本，需要从源码编译（需解决 ARM64 SIMD 兼容性问题）

3. **性能考虑**
   - 当前为同步渲染，大量请求可能导致阻塞
   - 后续可考虑实现异步队列（Celery/Redis Queue）
...
curl -X POST http://localhost:8081/api/render/solo \
  ...
  -d '{
    "image_path": "test.jpg",
    "solo_param": "hsl_green",
    "hsl": {"green": {"hue": "+15", "saturation": "+20"}}
  }'
```

#### 问题 8：高保真渲染成功后前端预览图像没有改变 ✅

**问题描述**：
- 后端返回渲染成功，`rendered_url` 为 `/static/rendered/ccbe6e36ccd46638616844a66363f3f9.jpg`
- 前端显示"渲染成功"，但预览图像没有任何改变
- 用户无法看到渲染后的图片效果

**排查过程**：
1. **检查后端渲染**：
   - XMP 文件生成正确，包含所有参数（exposure, temperature, filmicrgb, hazeremoval 等）
   - Darktable CLI 命令执行成功，渲染文件已生成
   - 渲染文件存在于 `storage/cache/rendered/` 目录

2. **检查前端显示逻辑**：
   - 前端设置了 `hiFiRenderedUrl` 和 `showHiFiResult` 状态
   - 但 `LivePreviewCanvas` 始终使用 `userImageUrl`，没有使用 `hiFiRenderedUrl`
   - 前端没有切换到显示渲染后的图片

3. **检查静态文件服务**：
   - 后端没有配置静态文件服务，无法访问 `/static/rendered/` 路径
   - 前端无法加载渲染后的图片

4. **检查 URL 处理**：
   - 后端返回的是相对路径 `/static/rendered/...`
   - 前端需要将其转换为完整的后端 URL（`http://localhost:8081/static/rendered/...`）

**修复方案**：

1. **修复前端显示逻辑** (`src/components/analysis/LightroomPanel.tsx`)：
   - 当 `showHiFiResult` 为 true 且 `hiFiRenderedUrl` 存在时，显示渲染后的图片（不使用 WebGL）
   - 添加切换按钮，让用户可以在 WebGL 预览和高保真渲染结果之间切换
   - 修复所有对比模式（none、split、slider）的显示逻辑

2. **添加后端静态文件服务** (`server_py/app/main.py`)：
   - 使用 `StaticFiles` 挂载 `storage/cache/rendered` 目录到 `/static/rendered` 路径
   - 确保目录存在，并记录日志

3. **修复前端 URL 处理** (`src/src/lib/hifiRenderService.ts`)：
   - 将相对路径转换为完整的后端 URL
   - 如果 `rendered_url` 以 `/` 开头，拼接后端基础 URL（`http://localhost:8081`）

**修复位置**：
- `src/components/analysis/LightroomPanel.tsx` - 修复显示逻辑，添加切换按钮
- `server_py/app/main.py` - 添加静态文件服务
- `src/src/lib/hifiRenderService.ts` - 修复 URL 处理

**经验教训**：
- 前端状态管理和 UI 显示需要保持一致
- 静态文件服务必须正确配置，否则前端无法访问资源
- 相对路径和绝对路径的处理需要统一规范
- 渲染结果应该自动显示，而不是需要用户手动切换

#### 问题 9：Darktable 渲染后图片没有变化（XMP 格式不兼容）✅

**问题描述**：
- 后端返回渲染成功，但渲染后的图片与原图完全相同
- Darktable CLI 执行成功，但显示 `[empty history stack]`
- 这意味着 Darktable 没有读取到任何编辑历史

**排查过程**：
1. **检查 Darktable CLI 输出**：
   ```
   [dt_init] SSE2 instruction set is unavailable.
   [export_job] exported to `/app/output/test.jpg'
   [empty history stack]  <-- 关键错误
   ```
...
2. **分析 XMP 格式差异**：
     ...
     <darktable:history>
       <rdf:Seq>
         <rdf:li>
           ...
           <darktable:params>
             <darktable:exposure>0.5</darktable:exposure>
           </darktable:params>
         </rdf:li>
       </rdf:Seq>
     </darktable:history>
   ...
   - **Darktable 期望的格式**：
     ```xml
     <rdf:Description
       darktable:history_operation="exposure"
       darktable:history_enabled="1"
       darktable:history_modversion="7"
       darktable:history_params="BASE64_ENCODED_BINARY_DATA"
       darktable:history_blendop_params="BASE64_ENCODED_BINARY_DATA"
       darktable:history_num="0"
       darktable:history_end="1">
     </rdf:Description>
     ```

3. **根本原因**：
   - Darktable 的参数是**二进制格式**，经过 **base64 编码**
   - 每个模块的参数结构是 Darktable 特定的 C 结构体
   - 这是一个复杂的工程问题，不是简单的 XML 格式问题

**解决方案**：
创建 Python 图像渲染引擎作为替代方案，使用 OpenCV/NumPy 实现 Lightroom 风格的图像调整。

**新增文件**：
- `server_py/app/services/python_render_engine.py` - Python 图像渲染引擎

**实现的调整**：
1. 曝光 (Exposure) - 使用 2^EV 公式
2. 对比度 (Contrast) - 以 0.5 为中心点
3. 高光/阴影 (Highlights/Shadows) - 基于亮度蒙版
4. 白色/黑色 (Whites/Blacks) - 色阶调整
5. 色温/色调 (Temperature/Tint) - RGB 通道调整
6. 饱和度/自然饱和度 (Saturation/Vibrance) - HSV 空间调整
7. 清晰度 (Clarity) - 高通滤波局部对比度
8. 去雾 (Dehaze) - 简化的暗通道先验算法
9. HSL 调整 - 基于色相范围的选择性调整
10. 色彩分级 - 基于亮度蒙版的着色

**修改的文件**：
- `server_py/app/services/hifi_render_service.py`
  - `render()` 方法改用 Python 渲染引擎
  - 保留 `render_with_darktable()` 方法供未来使用

**经验教训**：
- Darktable 的 XMP 格式非常复杂，需要深入研究其 C 源码才能正确实现
- Python 图像处理可以作为快速替代方案
- 渲染质量可能略低于 Darktable/Lightroom，但可以满足基本需求

**后续优化方向**：
1. 研究 Darktable 源码，实现正确的 XMP 格式
2. 使用 rawpy/LibRaw 实现更专业的 RAW 处理
3. 添加更多调整选项（曲线、局部调整等）

---

## 问题 10：Python 渲染引擎校准 v2 (2024-12)

**问题描述**：系统渲染结果与 Lightroom 调色存在较大偏差
...
| 模块 | 参数 | 原值 | 校准后 | 说明 |
|------|------|------|--------|------|
| 白平衡 | temp_factor | 0.6 | **0.85** | 增强冷/暖色调效果 |
| 白平衡 | tint_factor | 0.4 | **0.6** | 增强绿/洋红效果 |
| 白平衡 | 冷色调绿通道增益 | 0.3 | **0.5** | 增强青色 |
| 对比度 | gamma_dark | 0.5 | **0.6** | S 曲线陡峭度 |
| 对比度 | gamma_light | 0.3 | **0.4** | S 曲线陡峭度 |
| 黑位 | offset | 0.1 | **0.15** | 增强 Faded Black |
| 去雾 | omega | 0.6 | **0.45** | 防止过度去雾 |
| 去雾 | 透射率下限 | 0.15 | **0.3** | 防止高光溢出 |
| HSL | 绿色色相系数 | 1.0 | **1.8** | 绿色偏青关键 |
| HSL | 蓝色色相系数 | 1.0 | **1.5** | 蓝色偏青 |
| HSL | 饱和度系数 | 1.5 | **1.3** | 避免过饱和 |
| 色彩分级 | 着色强度 | 0.5 | **0.6** | 增强着色效果 |

**新增功能**：
- 添加相机校准 (Calibration) 支持
- 支持红/绿/蓝三原色的色相和饱和度偏移
- 支持阴影色调 (shadows_tint)

**修改文件**：
- `server_py/app/services/python_render_engine.py`
  - 更新 `_apply_white_balance()` - 校准 v2
  - 更新 `_apply_contrast()` - S 曲线增强
  - 更新 `_apply_whites_blacks()` - 安全边界 + Faded Black
  - 更新 `_apply_dehaze()` - 修复溢出
  - 更新 `_apply_hsl()` - 绿色偏青增强
  - 更新 `_apply_tint()` - 着色强度增强
  - 新增 `_apply_calibration()` - 相机校准

---

## 问题 11：Gemini 输出参数偏灰校准 (2024-12)

**问题描述**：Gemini 生成的调色参数过于保守，导致渲染结果偏灰

**根因分析**：Prompt 模板中存在错误指导：
1. ❌ "Dehaze 必须给负值 (-10 ~ -30) 以增加日系空气感" - **完全错误！**
   - 正确：Dehaze **正值**是去雾（增加通透），**负值**是加雾（更灰）
2. ❌ 对比度限制过于严格 (-10 ~ +10)，导致画面过于平淡
3. ❌ 缺少"防止发灰"的强制约束

**修复内容**：

### Prompt 模板修复 (`prompt_template.py`)

| 原错误 | 修复后 |
|--------|--------|
| Dehaze -15~-30 (负值!) | **Dehaze +15~+40 去雾（正值!）** |
| "Dehaze 必须给负值" | **"如果原图灰蒙蒙，必须用正值 Dehaze +15~+40 去雾！"** |
| 对比度 -10 ~ +10 | **对比度 -15 ~ +15** |

### 新增 "防灰协议" (ANTI-GREY PROTOCOL)
...
```
防灰检查（最重要！）：
- 画面是否发灰/雾蒙蒙？→ 立刻增加 Dehaze +15~+40
- 暗部是否发闷？→ 立刻提亮 Shadows +30~+60
- 颜色是否寡淡？→ 立刻增加 Vibrance +15~+30
```

**修改文件**：
- `server_py/app/services/prompt_template.py`
  - 修复 "元素调色手册" 中的去雾方向错误
  - 修复 "绝对禁忌" 中的去雾指导错误
  - 新增 "防止发灰 (ANTI-GREY PROTOCOL)" 约束
  - 更新 Step 5 虚拟审图检查

---

## 问题 12：仿色校准引擎实现 (2024-12)

**问题描述**：Gemini 输出的参数虽然符合 Prompt 要求，但可能因为 AI 的"保守性"导致实际渲染效果与参考图存在差距。需要引入基于统计矩匹配的校准算法，自动优化参数。

**设计原则**：
1. **Gemini 负责"理解"**：AI 负责理解图片风格、识别关键特征、给出定性判断（如暖色调还是冷色调）
2. **Python 算法负责"定量"**：校准引擎负责精确计算参数值，使用统计矩匹配方法找到最优参数
3. **闭环验证**：使用虚拟 Lightroom 引擎模拟渲染，在 LAB 色彩空间计算与参考图的差距

**实现方案**：

### 1. 虚拟 Lightroom 引擎 (`VirtualLREngine`)

模拟 Lightroom 的核心图像处理算法，用于校准循环中的参数验证：

- **白平衡**：RGB 通道增益模拟（temp/tint）
- **曝光**：摄影学 EV 公式 `2^exposure`
- **高光/阴影/白色/黑色**：基于亮度区间的调整
- **对比度**：S 型曲线
- **饱和度**：HSV 空间 S 通道调整

### 2. 校准引擎 (`CalibrationEngine`)

使用统计矩匹配方法校准参数：

- **下采样**：256px 下采样加速计算（控制处理时间在 500ms - 1s 级别）
- **统计特征**：计算参考图的 LAB 三通道 Mean + Std（6 维特征向量）
- **优化算法**：Powell 算法（无需导数，鲁棒性强）
- **加权 Loss**：色相权重 > 亮度权重（符合"仿色"目标）
...
Part2 分析完成
    ↓
格式化数据 (analysis_formatter)
    ↓
校准引擎自动运行 (calibration_engine)
    ├─ 解码用户图和参考图
    ├─ 计算参考图统计特征
    ├─ 解析 Gemini 参数作为初始值
    ├─ 优化算法求解最优参数
    └─ 覆盖 structured_result.sections.lightroom.structured.basic
    ↓
存入数据库
    ↓
前端显示校准后的参数
...
- **覆盖 Gemini 原始参数**：校准后的参数直接覆盖 `basic` 字段
- **保留 reason 字段**：在原始 reason 前添加 `[校准后]` 标记
- **前端只显示最终值**：前端代码无需修改，直接读取 `basic` 字段即可

### 5. 校准元数据

校准结果存入 `structured_result.meta.calibration`，包含：
- `status`: 校准状态（success/failed）
- `initial_loss`: 初始 Loss 值
- `final_loss`: 最终 Loss 值
- `improvement`: Loss 改善百分比
- `iterations`: 优化迭代次数
- `original_params`: 原始参数
- `calibrated_params`: 校准后参数

**修改文件**：
- `server_py/app/services/calibration_engine.py`（新增）
  - `VirtualLREngine`: 虚拟 Lightroom 引擎
  - `CalibrationEngine`: 校准引擎主类
  - `calibrate_lightroom_params`: 便捷函数
- `server_py/app/routes/analyze.py`
  - 在 `_run_part2_analysis_job()` 中集成校准引擎
  - 在格式化完成后、数据库更新前调用校准
  - 覆盖 `structured_result.sections.lightroom.structured.basic`

**测试结果**：
- 测试 Loss 改善：74-81%
- 校准时间：5-8 秒（256px 下采样）
- 参数格式：完全兼容前端数据读取路径

---

## 问题 13：Gemini API 503 错误处理优化 (2024-12)

**问题描述**：Gemini API 返回 503 服务过载错误时，错误处理代码出现 `KeyError: "'error'"`，导致前端收到 500 错误。

**根因分析**：
1. Gemini API 返回的错误信息包含字典格式：`{'error': {'code': 503, 'message': '...'}}`
2. 错误处理代码直接将错误字符串传递给 `error_response`，导致特殊字符解析错误
3. 503 错误未被识别为可重试错误，未进行自动重试

**修复内容**：

### 1. Gemini 服务层 (`gemini_service.py`)

- **识别 503 错误为可重试错误**：在 `is_retryable` 判断中添加 503/UNAVAILABLE/overloaded 检测
- **自动重试**：503 错误会触发指数退避重试（1秒、2秒、4秒）

### 2. 路由层 (`analyze.py`)

- **503 错误特殊处理**：检测到 503 错误时，返回用户友好的错误消息
- **错误消息提取**：从错误详情中提取用户友好的消息，避免直接使用包含字典的字符串
- **正则提取**：使用正则表达式从错误字符串中提取 `message` 字段

**修改文件**：
- `server_py/app/services/gemini_service.py`
  - 更新 `is_retryable` 判断逻辑，添加 503 错误检测
- `server_py/app/routes/analyze.py`
  - 添加 503 错误特殊处理
  - 改进错误消息提取逻辑，避免 KeyError

---

## 问题 14：Gemini API 超时配置不一致 (2024-12)

**问题描述**：`.env` 文件中的 `GEMINI_TIMEOUT_MS` 设置为 120000 毫秒（120秒），但开发方案明确要求应该是 180000 毫秒（180秒）。实际调用中，Gemini API 耗时 120.01 秒，刚好超过 120 秒限制导致超时。

**根因分析**：
1. `.env` 文件中的超时配置与开发方案不一致
2. 开发方案明确说明：Part1 分析可能需要 60-70 秒，AI 诊断可能需要 70+ 秒，考虑到网络延迟和波动，应设置为 180 秒
3. 实际调用耗时 120.01 秒，说明 120 秒确实不够

**修复内容**：

### 1. 更新 `.env` 配置

- **修复前**：`GEMINI_TIMEOUT_MS=120000`（120秒）
- **修复后**：`GEMINI_TIMEOUT_MS=180000`（180秒）

### 2. 验证配置生效

- 后端服务重启后，日志显示：`timeout=180000ms`，确认配置已生效
- 前端超时设置：200 秒（3分20秒），确保覆盖后端 180 秒超时，并留出网络延迟缓冲

### 3. 配置一致性检查

- ✅ 后端配置：`config.py` 默认值 180000 毫秒
- ✅ `.env` 文件：已更新为 180000 毫秒
- ✅ 前端配置：Part1 分析接口 200 秒超时（符合开发方案要求）
- ✅ 开发方案：明确说明应为 180 秒

**修改文件**：
- `server_py/.env`
  - 更新 `GEMINI_TIMEOUT_MS=180000`（从 120000 改为 180000）

**注意事项**：
- 超时配置必须与开发方案保持一致
- 前端超时应大于后端超时，留出网络延迟缓冲
- 如果 Gemini API 响应时间持续超过 180 秒，需要进一步分析原因（可能是网络问题、API 负载过高等）
...
## 问题 15：仿色校准引擎未生效 - basic 字段被 Pydantic Schema 过滤 (2024-12-01)

**问题描述**：用户反馈 LR 面板中的数据和 Gemini 日志一致，说明校准算法没有起作用。检查日志发现 `【仿色校准】⚠️ 未找到 basic_panel，跳过校准`。

**根因分析**：
1. `_format_lightroom` 函数确实返回了 `basic` 字段（在 `structured.basic` 中）
2. 但是 `LightroomStructuredSchema`（Pydantic Schema）中**没有定义 `basic` 字段**
3. 当 Pydantic 验证时，由于 Schema 中没有 `basic` 字段，它会被自动丢弃
4. 导致校准引擎在 `structured_result.sections.lightroom.structured.basic` 中找不到数据

**修复方案**：
1. 在 `LightroomStructuredSchema` 中添加 `basic` 字段定义
2. 字段类型：`Optional[Dict[str, Dict[str, str]]]`（字典的字典，键是参数名，值是包含 `range`、`note`、`reason` 的字典）
3. 添加详细的调试日志，便于排查问题

**修改文件**：
- `server_py/app/schemas/analysis_schemas.py`
  - 在 `LightroomStructuredSchema` 中添加 `basic` 字段
- `server_py/app/routes/analyze.py`
  - 添加详细的调试日志，记录 `basic_panel` 的检查过程

**验证方法**：
1. 运行 Part2 分析
2. 检查日志中是否出现 `【仿色校准】开始校准 Lightroom 基础参数`
3. 检查日志中是否出现 `【仿色校准】✅ 校准成功！Loss 改善`
4. 检查前端 LR 面板中的参数是否与 Gemini 原始输出不同（说明校准生效）

---

## 问题 16：扩展校准引擎支持所有 Lightroom 参数 (2024-12-01)

**问题描述**：用户反馈不仅仅是 `basic_panel`，其他参数如 `tone_curve`、`hsl`、`color_grading`、`calibration`、`effects` 都没有经过校准算法计算。

**根因分析**：
1. 当前校准引擎只处理 `basic_panel` 参数
2. `VirtualLREngine` 只支持基础参数，不支持其他参数
3. 校准函数 `calibrate_lightroom_params` 只接受 `basic_panel` 作为输入

**修复方案**：
1. **扩展 VirtualLREngine**：
   - 添加 `tone_curve` 支持（RGB 曲线控制点）
   - 添加 `hsl` 支持（8 种颜色的色相/饱和度/明度调整）
   - 添加 `color_grading` 支持（高光/中间调/阴影的色彩分级）
   - 添加 `calibration` 支持（相机校准，红/绿/蓝原色调整）
   
2. **创建完整校准函数**：
   - 新增 `calibrate_all_lightroom_params` 函数
   - 接受完整的 `lightroom.structured` 数据作为输入
   - 策略：先校准基础参数（最重要），其他参数保持 Gemini 输出不变（因为优化复杂度高）

3. **修改 analyze.py**：
   - 使用 `calibrate_all_lightroom_params` 替代 `calibrate_lightroom_params`
   - 覆盖完整的 `lightroom.structured` 数据

**修改文件**：
- `server_py/app/services/calibration_engine.py`
  - 扩展 `VirtualLREngine.apply` 支持所有参数
  - 新增 `calibrate_all` 方法
  - 新增 `calibrate_all_lightroom_params` 便捷函数
- `server_py/app/routes/analyze.py`
  - 使用 `calibrate_all_lightroom_params` 替代 `calibrate_lightroom_params`
  - 覆盖完整的 `lightroom.structured` 数据

**注意事项**：
- 当前策略：只优化基础参数（basic_panel），其他参数保持 Gemini 输出不变
- 原因：优化所有参数（tone_curve 控制点 + hsl 24 个参数 + color_grading 9 个参数 + calibration 6 个参数）会导致计算时间过长（可能超过 10 秒）
- 未来优化：可以考虑分阶段优化或限制迭代次数来优化所有参数

---

## 问题 17：实现完整参数优化（所有 Lightroom 参数）(2024-12-01)

**问题描述**：用户要求优化所有 Lightroom 参数，不仅仅是 `basic_panel`，还包括 `toneCurve`、`hsl`、`colorGrading`、`calibration`。

**实现方案**：
1. **参数编码**：
   - 基础参数：9 个（exposure, temp, tint, contrast, saturation, highlights, shadows, whites, blacks）
   - 色调曲线：5 个 y 值（x 值固定为 [0, 64, 128, 192, 255]）
   - HSL 调整：8 种颜色 × 3 个参数 = 24 个参数
   - 色彩分级：3 个区域 × 2 个参数（hue, saturation）= 6 个参数（luminance 保持为 0）
   - 相机校准：3 个原色 × 2 个参数 = 6 个参数
   - **总计：50 个优化参数**

2. **优化策略**：
   - 使用 Powell 算法（不需要导数，鲁棒性强）
   - 限制迭代次数为 50 次，容差为 1e-2，控制计算时间在 5-10 秒内
   - 一次性优化所有参数（不采用分阶段优化，因为参数之间有相互影响）
...
**修改文件**：
- `server_py/app/services/calibration_engine.py`
  - 新增 `_parse_all_params` 方法：解析所有 Gemini 参数
  - 新增 `_encode_all_params` 方法：将参数编码为优化器向量
  - 新增 `_decode_all_params` 方法：将优化结果解码回原始格式
  - 新增 `_params_dict_to_virtual_lr_params` 方法：转换为 VirtualLREngine 格式
  - 重写 `calibrate_all` 方法：实现完整优化逻辑

**性能考虑**：
- 参数数量：50 个
- 优化迭代次数：50 次（限制）
- 预计计算时间：5-10 秒（取决于图片大小和优化收敛速度）
- 如果计算时间过长，可以进一步减少迭代次数或采用分阶段优化

**验证方法**：
1. 运行 Part2 分析
2. 检查日志中是否出现 `【校准引擎-完整】开始优化所有参数...`
3. 检查日志中是否出现 `【校准引擎-完整】✅ 校准完成，Loss 改善`
4. 检查前端 LR 面板中的所有参数是否与 Gemini 原始输出不同（说明校准生效）
...
1. **Gemini API 503 错误处理优化**
   - 问题：503 错误未被识别为可重试错误，错误消息解析出现 `KeyError: "'error'"`
   - 修复：在所有 Gemini API 调用处添加 503 错误特殊处理和错误消息提取逻辑
   - 文件：`server_py/app/services/gemini_service.py`, `server_py/app/routes/analyze.py`, `server_py/app/routes/simulate.py`

2. **超时配置不一致**
   - 问题：`.env` 文件中的超时设置为 120 秒，但开发方案要求 180 秒
   - 修复：更新 `.env` 文件中的 `GEMINI_TIMEOUT_MS=180000`
   - 文件：`server_py/.env`

3. **重复的异常处理**
   - 问题：Part1 分析中有两个 `except TimeoutError` 块，第二个永远不会被执行
   - 修复：删除重复的异常处理，保留一个并添加详细注释
   - 文件：`server_py/app/routes/analyze.py`

4. **前端错误消息中英文支持**
   - 问题：前端错误消息硬编码为英文，不支持中英文切换
   - 修复：添加翻译键 `analysis.part1_failed`，前端使用 `t()` 函数显示翻译消息
   - 文件：`src/App.tsx`, `src/src/contexts/LanguageContext.tsx`

5. **仿色校准引擎未生效 - basic 字段被 Pydantic Schema 过滤**
   - 问题：`_format_lightroom` 返回了 `basic` 字段，但 Pydantic Schema 验证时被丢弃，导致校准引擎找不到数据
   - 修复：在 `LightroomStructuredSchema` 中添加 `basic` 字段定义，添加详细调试日志
   - 文件：`server_py/app/schemas/analysis_schemas.py`, `server_py/app/routes/analyze.py`

### 代码一致性检查

✅ **错误处理一致性**：
- 所有 Gemini API 调用都包含 503 错误处理
- 所有错误处理都包含错误消息提取逻辑（使用正则表达式提取 `message` 字段）
- 所有超时错误都显示明确的超时时间（180 秒）
...
- **构建说明**：`server_py/docker/BUILD.md`（Darktable 相关，已废弃）
- **集成测试**：`server_py/docker/INTEGRATION_TEST.md`（Darktable 相关，已废弃）
- **部署总结**：`server_py/docker/DEPLOYMENT_SUMMARY.md`（Darktable 相关，已废弃）
- **验证脚本**：`server_py/docker/verify.sh`（Darktable 相关，已废弃）
- **测试脚本**：`server_py/docker/test_render.sh`（Darktable 相关，已废弃）

---

## 问题 18：渲染引擎实现说明 - Python 引擎替代 Darktable (2024-12-01)

**问题描述**：用户发现 Docker 界面中没有 darktable 容器，询问后端 darktable 是如何实现的。

**实际情况**：
1. **当前实现**：后端**不再使用 Darktable**，而是使用 **Python 图像处理引擎**（`python_render_engine.py`）
2. **实现位置**：
   - `server_py/app/services/python_render_engine.py`：Python 渲染引擎核心实现
   - `server_py/app/services/hifi_render_service.py`：高保真渲染服务（调用 Python 引擎）
   - `server_py/app/routes/render.py`：渲染 API 端点

**变更历史**：
1. **最初方案**：尝试使用 Darktable CLI 进行高保真渲染
2. **遇到的问题**：Darktable XMP 格式过于复杂，需要 base64 编码的二进制参数，难以正确映射 Lightroom 参数
3. **解决方案**：切换到纯 Python 实现，使用 OpenCV/NumPy 模拟 Lightroom 调整
...
**保留的 Darktable 相关代码**（仅作为参考，未使用）：
- `server_py/app/services/darktable_xmp_generator.py`：XMP 生成器（保留作为参考）
- `server_py/docker/Dockerfile.darktable`：Docker 镜像构建文件（保留作为参考）
- `server_py/app/services/hifi_render_service.py` 中的 `render_with_darktable` 方法：保留作为备用

**修改文件**：
- `server_py/app/services/hifi_render_service.py`：更新类注释，说明当前使用 Python 引擎
- `server_py/app/routes/render.py`：更新 API 文档注释，说明当前实现
- `开发方案.md`：添加"重要更新"部分，说明渲染引擎变更

**验证方法**：
1. 检查日志：运行高保真渲染时，日志应显示"使用 Python 渲染引擎进行渲染..."
2. 检查代码：`hifi_render_service.py` 的 `render` 方法调用 `python_render_engine.render`
3. 检查 Docker：确认没有 `pstyle-darktable` 容器（这是正常的，因为不再需要）

---

## 问题 19：色温/色调（temp/tint）参数缺失问题 (2024-12-01)

### 问题描述

用户反馈 LR 面板中数据有变化，但缺少色温（temp）和色调（tint）字段。

### 问题根因分析

1. **数据来源不一致**：
   - Gemini 有时将 temp/tint 输出到 `lightroom_workflow.basic_panel` 中（新结构）
   - 有时将 temp/tint 输出到 `lightroom_panels[].params` 中（旧结构，以中文名 "色温"/"色调" 显示）

2. **构建逻辑缺陷**：
   - `_format_lightroom` 方法只从 `basic_panel`（新结构）构建 `basic_dict`
   - 没有回退到 `panels` 数组（旧结构）获取缺失的参数
   - 导致当 Gemini 将 temp/tint 放在 panels 中时，`basic_dict` 中缺少这些参数

### 修复方案

在 `analysis_formatter.py` 的 `_format_lightroom` 方法中添加**回退机制**：

```python
# 1. 首先从 basic_params（新结构 basic_panel）提取
for param in basic_params:
    param_name_en = map_cn_to_en(param.get("name", ""))
    if param_name_en:
        basic_dict[param_name_en] = {...}

# 2. 【回退机制】如果 basic_dict 缺少 temp/tint，从 panels 数组中补充
required_params = ['temp', 'tint', 'exposure', 'contrast', ...]
missing_params = [p for p in required_params if p not in basic_dict]

if missing_params and panels:
    for panel in panels:
        if panel.get("title") in ["基本面板", "基础面板", "Basic"]:
            for param in panel.get("params", []):
                param_name_en = map_cn_to_en(param.get("name", ""))
                if param_name_en in missing_params:
                    basic_dict[param_name_en] = {...}
```
...
| 文件 | 修改内容 |
|------|----------|
| `server_py/app/services/analysis_formatter.py` | 在 `_format_lightroom` 方法中添加从 panels 数组补充 temp/tint 的回退机制 |
...
Gemini 输出
    │
    ├── lightroom_workflow.basic_panel（新结构）
    │       ↓
    │   basic_params 列表
    │       ↓
    │   basic_dict（第一优先级）
    │
    └── lightroom_panels（旧结构）
            ↓
        panels 数组（包含中文参数名如"色温"、"色调"）
            ↓
        回退补充到 basic_dict（仅补充缺失的参数）
            ↓
        最终 basic_dict（前端读取）
```
...
```
后端返回: structured_result.sections.lightroom.structured.basic
         ↓
dataAdapter.ts: structured.basic
         ↓
safeBasicPanel: { temp, tint, exposure, ... }
         ↓
LightroomPanel.tsx: data.basic_panel.temp / data.basic_panel.tint
```

### 校准引擎数据流

```
1. Gemini 输出 → _format_lightroom() → basic 字段（包含 temp/tint）
2. basic 字段 → Pydantic 验证 → structured_result.lightroom.structured.basic
3. calibrate_all_lightroom_params() 读取 basic 字段
4. 校准后覆盖 basic 字段 → 保存到数据库
5. 前端读取 basic 字段 → 显示校准后的 temp/tint
```
...
1. **查看后端日志**：
   ```
   【_format_lightroom】从 basic_params 提取: X 个参数, keys=[...]
   【_format_lightroom】✅ 从 panels 补充参数 temp: +20
   【_format_lightroom】✅ 从 panels 补充参数 tint: +5
   【_format_lightroom】✅ 构建 basic 字段完成: temp存在=True, tint存在=True
   ```

2. **检查数据库**：
   ```sql
   SELECT JSON_EXTRACT(structured_result, '$.sections.lightroom.structured.basic.temp') 
   FROM analysis_tasks 
...
3. **前端控制台**：
   ```
   [dataAdapter] 🔍 Lightroom basic_panel 数据检查: keyParams.temp = {...}
   [LightroomPanel] 最终色温/色调值: { tempValue: 5520, tintValue: 5 }
   ```

### 相关问题

- 问题 15：仿色校准引擎未生效 - basic 字段被 Pydantic Schema 过滤
- 问题 16：仿色校准引擎扩展 - 优化所有 Lightroom 参数
- 问题 20：校准引擎 NumPy 数组广播错误（见下文）

---

## 问题 20：校准引擎 NumPy 数组广播错误 (2024-12-01)

### 问题描述

校准引擎执行时抛出异常：
```
ValueError: non-broadcastable output operand with shape (256,256) doesn't match the broadcast shape (256,256,256)
```

导致所有校准都失败，Gemini 原始参数未经校准直接传递给前端。

### 问题根因

在 `VirtualLREngine.apply` 方法的色彩分级（Color Grading）处理部分：

```python
# 错误代码
lab[:, :, 1] += h_mask[:, :, np.newaxis] * a_shift
```

- `h_mask` 是 2D 数组 `(256, 256)`
- `h_mask[:, :, np.newaxis]` 变成 3D 数组 `(256, 256, 1)`
- 乘以标量 `a_shift` 后仍是 `(256, 256, 1)`
- 无法赋值给 2D 数组 `lab[:, :, 1]`（形状 `(256, 256)`）

### 修复方案

移除不必要的 `np.newaxis`，直接使用 2D 数组：
...
| 文件 | 修改内容 |
|------|----------|
| `server_py/app/services/calibration_engine.py` | 修复 `VirtualLREngine.apply` 方法中 3 处数组广播错误（highlights、midtones、shadows） |
...
- Loss 改善: **42.96%**
- 参数变化示例：
  - temp: +400 → +50（校准后减少 350）
  - contrast: +15 → +50（校准后增加 35）
  - exposure: +0.40 → +2.00（校准后增加 1.6）

### 影响范围

此 bug 导致**所有历史任务的校准都失败**，数据库中存储的是 Gemini 原始参数。用户需要**重新运行分析**才能获得校准后的参数。

---

## 问题 21：Part1 分析超时错误处理优化 (2024-12-01)

### 问题描述

用户反馈 Part1 分析请求超时（超过 180 秒），前端显示 500 Internal Server Error，错误消息为 "AI 分析超时（超过 180 秒）"。

### 问题根因分析

1. **错误码不正确**：
   - 超时错误使用了 `ErrorCode.INTERNAL_ERROR`（HTTP 500）
   - 应该使用 `ErrorCode.GEMINI_TIMEOUT`（HTTP 504 Gateway Timeout）

2. **日志不够详细**：
   - 超时错误缺少详细的诊断信息（如请求详情、图片数量等）
   - 无法快速定位超时原因

3. **性能监控缺失**：
   - 没有记录 Gemini API 调用的实际耗时
   - 无法判断是 Gemini API 慢还是其他问题
...
1. **修复错误码**：
   ```python
   # 修复前
   raise error_response(ErrorCode.INTERNAL_ERROR, f"AI 分析超时（超过 180 秒）...")
   
   # 修复后
   raise error_response(ErrorCode.GEMINI_TIMEOUT, f"AI 分析超时（超过 {timeout} 秒）...")
   ```

2. **增强日志记录**：
   - 记录 Gemini API 调用开始时间和实际耗时
   - 记录请求详情（parts 数量、图片数量）
   - 记录超时时的重试次数

3. **添加性能监控**：
   - 在调用 Gemini API 前后记录时间戳
   - 计算实际耗时，便于性能分析
...
| 文件 | 修改内容 |
|------|----------|
| `server_py/app/routes/analyze.py` | 修复超时错误码为 GEMINI_TIMEOUT，添加详细日志和性能监控 |
| `server_py/app/services/gemini_service.py` | 增强超时错误日志，记录请求详情 |

### 错误码映射

| 错误码 | HTTP 状态码 | 说明 |
|--------|-------------|------|
| `GEMINI_TIMEOUT` | 504 Gateway Timeout | Gemini API 调用超时 |
...
修复后的日志格式：
```
【Part1 分析】准备调用 Gemini API，contents parts 数量: 5
【Part1 分析】Gemini 超时设置: 180.0秒（180000毫秒）
【Part1 分析】Gemini 模型: gemini-3-pro-preview
【Part1 分析】✅ Gemini API 调用成功，耗时: 65.23秒，响应长度: 15234 字符
```

超时时的日志：
```
【Gemini part1】❌ 调用超时，耗时: 180.05s，超时设置: 180.0秒
【Gemini part1】超时详情: 模型=gemini-3-pro-preview, stage=part1, 已重试次数=0
【Gemini part1】请求详情: parts数量=5, 图片数量=2
【Part1 分析】❌ Gemini API 调用超时: Gemini API 调用超时（超过 180.0 秒）
【Part1 分析】超时详情: taskId=xxx, 超时设置=180.0秒
```

### 前端处理

前端已设置 200 秒超时（覆盖后端 180 秒），超时错误会正确显示给用户。

### 可能的原因

1. **Gemini API 响应慢**：图片数据过大或模型负载高
2. **网络问题**：代理服务器延迟或网络不稳定
3. **图片数据过大**：超过 20MB 的 Base64 数据可能导致处理时间过长

### 建议

如果超时问题持续：
1. 检查图片大小，建议压缩到 10MB 以下
2. 检查代理服务器状态（ClashX/Clash Verge 端口 7897）
3. 检查 Gemini API 服务状态
4. 查看后端日志获取更详细的诊断信息

---

## 问题 22：Part2 请求超时问题修复 (2024-12-02)

### 问题描述

用户反馈 Part2 请求超时（超过 10 秒），前端显示 `AbortError: signal is aborted without reason`，错误消息为 "Part2 分析请求超时（超过 10 秒）"。

### 问题根因分析

1. **前端超时时间过短**：
   - 初始设置为 10 秒超时
   - 虽然后端应该在 0.01 秒内返回，但考虑到网络延迟和可能的阻塞，10 秒可能不够

2. **缺少详细的性能监控**：
   - 前端没有记录 fetch 和 JSON 解析的耗时
   - 无法快速定位是网络问题还是解析问题
...
2. **增强前端性能监控**：
   - 记录 fetch 请求耗时（`fetchElapsed`）
   - 记录 JSON 解析耗时（`jsonParseElapsed`）
   - 记录总耗时（`totalElapsed`）
...
| 文件 | 修改内容 |
|------|----------|
| `src/src/lib/api.ts` | 将 Part2 超时时间从 10 秒增加到 30 秒，添加详细的性能监控日志（fetchElapsed、jsonParseElapsed、totalElapsed） |
| `server_py/app/routes/analyze.py` | 增强 Part2 响应日志，记录响应数据大小和预览 |

### 前端性能监控日志

修复后的日志格式：
```
[API Request] POST http://localhost:8081/api/analyze/part2
[API Response] POST http://localhost:8081/api/analyze/part2
  status: 200
  fetchElapsed: 45ms
  contentType: application/json
  contentLength: 89
[API Response Data] POST http://localhost:8081/api/analyze/part2
  code: 0
  message: ok
  jsonParseElapsed: 2ms
  totalElapsed: 47ms
```
...
修复后的日志格式：
```
【Part2 任务已提交后台】taskId=xxx, 请求处理耗时=0.01秒, 后台任务已创建, 即将返回响应
【Part2 请求完成】taskId=xxx, 总耗时=0.0123秒, 响应状态=processing, 响应大小=89 字符
【Part2 响应数据预览】taskId=xxx, 响应内容={"code":0,"message":"ok","data":{"taskId":"xxx","stage":"part2","status":"processing"}}
```

### 可能的原因

1. **网络延迟**：前端和后端之间的网络延迟导致响应没有及时到达
2. **浏览器阻塞**：浏览器可能因为其他原因阻塞了请求
3. **CORS 问题**：虽然 OPTIONS 请求成功，但实际响应可能被 CORS 拦截（但日志显示响应成功返回）

### 验证方法

1. **查看前端控制台**：
   - 检查 `[API Request]` 和 `[API Response]` 日志
   - 查看 `fetchElapsed` 和 `jsonParseElapsed` 时间
   - 如果 `fetchElapsed` 接近 30 秒，说明网络有问题

2. **查看后端日志**：
   - 检查 `【Part2 请求完成】` 日志的时间戳
   - 确认响应是否在 0.01 秒内返回

3. **网络测试**：
   ```bash
   curl -X POST http://localhost:8081/api/analyze/part2 \
     -H "Content-Type: application/json" \
     -H "Authorization: Bearer YOUR_TOKEN" \
...
## 问题 23：Part2 请求超时 - FastAPI BackgroundTasks 修复 (2024-12-02)

### 问题描述

用户反馈 Part2 请求仍然超时（超过 30 秒），即使前端超时时间已增加到 30 秒。后端日志显示请求耗时 66 秒，说明请求被阻塞。

### 问题根因分析

1. **使用了 `asyncio.create_task` 而不是 FastAPI 的 `BackgroundTasks`**：
   - `asyncio.create_task` 创建的任务可能被 FastAPI 等待
   - FastAPI 在响应返回后，可能会等待所有后台任务完成
   - 这导致前端请求被阻塞，直到后台任务完成

2. **FastAPI 推荐使用 `BackgroundTasks`**：
   - `BackgroundTasks` 确保响应立即返回
   - 后台任务在响应返回后执行
   - FastAPI 会正确跟踪任务状态，不会阻塞响应
...
3. 使用 `background_tasks.add_task` 替代 `asyncio.create_task`：
   ```python
   # 修复前：使用 asyncio.create_task（可能被 FastAPI 等待）
   ...
   # 修复后：使用 BackgroundTasks（确保响应立即返回）
...
| 文件 | 修改内容 |
|------|----------|
| `server_py/app/routes/analyze.py` | 使用 FastAPI 的 `BackgroundTasks` 替代 `asyncio.create_task`，确保响应立即返回 |

### 技术说明

1. **`BackgroundTasks` vs `asyncio.create_task`**：
   - `asyncio.create_task`：创建的任务可能被 FastAPI 等待，导致响应阻塞
   - `BackgroundTasks`：FastAPI 推荐的方式，确保响应立即返回，后台任务在响应返回后执行

2. **`BackgroundTasks.add_task` 支持 async 函数**：
   - FastAPI 的 `BackgroundTasks.add_task` 可以接受 async 函数
   - FastAPI 会自动处理 async 函数的执行
   - 不需要额外的包装函数
...
1. **查看后端日志**：
   - 检查 `【Part2 请求完成】` 日志的时间戳
   - 确认响应是否在 0.01 秒内返回（而不是 66 秒）

2. **查看前端控制台**：
   - 检查 `[API Response]` 日志中的 `fetchElapsed` 时间
   - 应该小于 1 秒（而不是 30 秒）

3. **功能测试**：
   - 触发 Part2 请求
   - 前端应该立即收到 `{ status: 'processing' }` 响应

---

## 问题 25：校准引擎数据源映射修复 - 支持从 Gemini 原始输出读取 (2024-12-02)

### 问题描述

用户反馈校准算法未执行，日志显示 `【仿色校准】⚠️ 未找到 lightroom_structured，跳过校准`。检查发现 `structured_result.sections.lightroom.structured` 不存在，但 Gemini 原始输出 `gemini_json.lightroom_workflow` 存在。

### 问题根因分析

1. **数据源单一**：
   - 校准引擎只从 `structured_result.sections.lightroom.structured` 读取数据
   - 如果 `format_part2` 格式化失败或数据丢失，校准引擎无法获取参数
   - Gemini 原始输出 `lightroom_workflow` 没有被利用

2. **数据格式不匹配**：
   - Gemini 输出格式：`lightroom_workflow.basic_panel`、`lightroom_workflow.hsl`（下划线命名）
   - 校准引擎期望格式：`lightroom.structured.basic`、`lightroom.structured.hsl`（驼峰命名）
   - 缺少格式转换函数
...
1. **新增转换函数 `_convert_lightroom_workflow_to_structured`**：
   - 将 Gemini 的 `lightroom_workflow` 格式转换为 `lightroom.structured` 格式
   - 支持 `basic_panel`、`tone_curve`、`hsl`、`color_grading`、`calibration` 字段
   - 处理字段名转换（下划线 → 驼峰，如 `color_grading` → `colorGrading`）
   - 保留所有元数据（`reason`、`note` 等）
...
3. **修复保存逻辑**：
   - 如果 `sections.lightroom` 不存在，先创建再保存校准结果
   - 确保校准后的参数能正确保存到 `structured_result`

### 修改的文件

| 文件 | 修改内容 |
|------|----------|
| `server_py/app/routes/analyze.py` | 新增 `_convert_lightroom_workflow_to_structured` 函数，修改校准逻辑支持双数据源 |

### 技术说明

1. **数据源优先级**：
   - 优先使用格式化后的数据（`structured_result`），因为已经过 Pydantic 验证
   - 如果格式化失败，回退到 Gemini 原始输出（`lightroom_workflow`）

2. **格式转换**：
   - Gemini 输出使用下划线命名（`color_grading`、`tone_curve`）
   - 校准引擎期望驼峰命名（`colorGrading`、`toneCurve`）
   - 转换函数负责字段名映射和数值解析

3. **元数据保留**：
   - 转换时保留所有元数据（`reason`、`note` 等）
   - 确保校准后的参数仍然包含原始说明

### 验证方法

1. **查看后端日志**：
   - 检查 `【仿色校准】✅ 从 structured_result 读取` 或 `【仿色校准】✅ 从 gemini_json 读取`
   - 检查 `【仿色校准】数据源: xxx` 日志，确认数据源
   - 检查 `【仿色校准】✅ 校准成功！Loss 改善` 日志

2. **功能测试**：
   - 触发 Part2 分析
   - 即使 `format_part2` 失败，校准引擎仍能从 `lightroom_workflow` 读取数据
   - 检查前端 LR 面板中的参数是否经过校准
...
## 问题 24：前端色温色调面板未显示 (2024-12-02)

### 问题描述

用户反馈前端 Lightroom 面板中没有显示色温（temp）和色调（tint）的控件，即使后端已经正确返回了这些数据。

### 问题根因分析

1. **`dataAdapter.ts` 中 `white_balance` 构建逻辑问题**：
   - 检查逻辑过于严格：只有当 `temp` 或 `tint` 的值不是默认值（`value !== 0` 且 `range !== "+0"`）时，才认为有值
   - 如果 `temp` 和 `tint` 是默认值（`{ value: 0, range: "+0" }`），`hasTempInBasic` 和 `hasTintInBasic` 都会是 `false`
   - 导致 `white_balance` 对象不会被构建，函数返回 `undefined`

2. **前端条件渲染问题**：
   - `LightroomPanel.tsx` 中使用 `{data.white_balance && (...)}` 条件渲染
   - 如果 `data.white_balance` 是 `undefined`，整个色温色调面板都不会显示
   - 即使数据是默认值，也应该显示UI组件，让用户可以调整

### 修复方案

#### 1. 修复 `dataAdapter.ts` 中的 `white_balance` 构建逻辑

**文件**: `src/src/lib/dataAdapter.ts`

**修改内容**:
1. 放宽检查条件：只要 `temp` 或 `tint` 字段存在（不是 `undefined`），就认为有值
2. 添加备用方案：如果 `basic_panel` 和 `whiteBalance` 都没有数据，返回默认值对象，而不是 `undefined`

```typescript
// 修复前：检查逻辑过于严格
const hasTempInBasic = tempFromBasic && (
  (tempFromBasic.value !== undefined && tempFromBasic.value !== 0) || 
  (tempFromBasic.range !== undefined && tempFromBasic.range !== "+0" && tempFromBasic.range !== "0")
);

// 修复后：只要字段存在就认为有值
const hasTempInBasic = tempFromBasic && tempFromBasic !== undefined;

// 修复前：如果没有数据，返回 undefined
return undefined;
...
3. 改进 `temp` 和 `tint` 值的提取逻辑，正确处理数字类型的 `value`：
```typescript
...
#### 2. 修复前端 `LightroomPanel.tsx` 中的条件渲染

**文件**: `src/components/analysis/LightroomPanel.tsx`

**修改内容**:
1. 移除条件渲染：将 `{data.white_balance && (...)}` 改为直接渲染，确保色温色调面板始终显示
2. 在 `displayData` 的构建中，确保即使 `data.white_balance` 是 `undefined`，也能使用默认值
...
// 修复前：使用 || 运算符（0 会被视为 falsy）
const origTemp = data.white_balance?.temp?.value || 5500;

// 修复后：使用 ?? 运算符（只有 undefined/null 才会使用默认值）
const origTemp = data.white_balance?.temp?.value ?? 5500;
```
...
| 文件 | 修改内容 |
|------|----------|
| `src/src/lib/dataAdapter.ts` | 修复 `white_balance` 构建逻辑，确保即使值是默认值也构建对象；添加备用方案返回默认值对象；修复变量名冲突（`result` -> `wbResult`）；添加防御性编程措施，在 `result.lightroom` 构建后检查并强制设置默认值 |
| `src/components/analysis/LightroomPanel.tsx` | 移除条件渲染，确保色温色调面板始终显示；改进 `displayData` 构建逻辑，使用 `??` 运算符 |

### 技术说明

1. **为什么即使值是默认值也应该显示UI**：
   - 用户体验：用户应该能够看到和调整所有参数，即使当前值是默认值
   - 数据一致性：确保前端UI始终有数据可显示，避免条件渲染导致的UI闪烁
   - 调试便利：即使后端数据有问题，前端也能显示默认值，便于调试

2. **变量名冲突修复**：
   - 在 `white_balance` 的 IIFE 中，局部变量 `result` 与外层 `result` 对象冲突
   - 修复：将局部变量 `result` 改为 `wbResult`，避免变量名冲突

3. **防御性编程措施**：
   - 在 `result.lightroom` 构建完成后，立即检查 `white_balance` 是否为 `undefined` 或 `null`
   - 如果是，强制设置为默认值对象，确保前端UI始终可以显示色温色调面板
   - 添加详细的调试日志，包括检查 `white_balance` 的类型、是否为 `null`、是否为 `undefined`，以及 `result.lightroom` 对象本身的状态

4. **`??` vs `||` 运算符的区别**：
   - `||`：如果左侧是 falsy 值（`0`、`""`、`false`、`null`、`undefined`），使用右侧值
   - `??`：只有左侧是 `null` 或 `undefined` 时，才使用右侧值
   - 对于数值 `0`，应该使用 `??` 而不是 `||`，因为 `0` 是一个有效的值

3. **默认值的选择**：
   - 色温默认值：`5500K`（Lightroom 标准默认值，接近日光）
   - 色调默认值：`0`（中性色调，不偏绿也不偏洋红）

### 验证方法

1. **查看前端控制台日志**：
   - 检查 `[dataAdapter] ✅ 最终 white_balance 对象:` 日志
   - 确认 `white_balance` 对象是否被正确构建
...
3. **功能测试**：
   - 触发 Part2 分析
   - 检查前端是否显示色温色调面板
   - 检查色温色调值是否正确显示（即使值是默认值）

### 相关代码位置

- **后端数据格式化**: `server_py/app/services/analysis_formatter.py` (第 3717-3763 行)
- **前端数据适配**: `src/src/lib/dataAdapter.ts` (第 1736-1857 行)
- **前端UI组件**: `src/components/analysis/LightroomPanel.tsx` (第 2918-2957 行)
...
- `BackgroundTasks` 是 FastAPI 推荐的后台任务处理方式
- 确保后台任务使用独立的数据库会话（已在 `_run_part2_analysis_job` 中实现）
- 后台任务的错误不会影响响应返回，但会在日志中记录
4. 考虑增加超时时间（但需要评估用户体验）
...
用户报告登录时前端卡在"正在验证凭据..."，Chrome DevTools 显示登录请求状态为 `(pending)`，最终超时。前端控制台显示 `AbortError: signal is aborted without reason` 和 `请求超时,请检查网络连接或稍后重试`。

### 问题原因

后端服务启动失败，导致无法处理任何请求。根本原因是 `server_py/app/services/hifi_render_service.py` 文件在 RawTherapee 迁移过程中引入了**缩进错误**（IndentationError），导致 Python 模块导入失败，FastAPI 应用无法正常启动。

**具体错误位置**：
- 第 242-243 行：`xmp_dir` 和 `pp3_path` 的缩进不正确
- 第 248-251 行：`container_name`、`input_image_name`、`pp3_filename`、`container_pp3_path` 的缩进不正确
- 第 258-261 行：`if copy_result.returncode != 0` 和 `except` 块的缩进不正确
- 第 288-314 行：`try-except` 块的缩进结构完全混乱

**错误信息**：
```
File "/Users/.../server_py/app/services/hifi_render_service.py", line 243
...
1. **修复缩进错误**：
   - 将所有在 `if self._check_docker_available():` 块内的代码统一缩进到正确的级别（4 个空格）
   - 修复 `try-except` 块的结构，确保 `try`、`except`、`else` 在同一缩进级别
   - 修复 `if-else` 块的结构，确保 `if` 和 `else` 在同一缩进级别
...
2. **代码结构修复**：
   ...
   if self._check_docker_available():
       self.logger.info(...)
   xmp_dir = base_path / self.config['xmp_dir']  # ❌ 缩进错误
       pp3_path = xmp_dir / f"{cache_key}.pp3"  # ❌ 缩进错误
   
   # 修复后（正确）：
   if self._check_docker_available():
       self.logger.info(...)
       xmp_dir = base_path / self.config['xmp_dir']  # ✅ 正确缩进
       pp3_path = xmp_dir / f"{cache_key}.pp3"  # ✅ 正确缩进
   ```

3. **验证修复**：
   - 使用 `read_lints` 工具检查语法错误
   - 重启后端服务，确认启动成功
   - 使用 `curl` 测试登录接口，确认可以正常响应

### 修复文件

| 文件路径 | 修复内容 |
|---------|---------|
| `server_py/app/services/hifi_render_service.py` | 修复第 237-318 行的缩进错误，确保所有代码块在正确的缩进级别 |

### 技术说明

1. **Python 缩进规则**：
   - Python 使用缩进来表示代码块，而不是大括号
   - 同一代码块内的所有语句必须有相同的缩进级别
   - 缩进错误会导致 `IndentationError`，这是语法错误，会阻止模块导入

2. **为什么会导致服务无法启动**：
   - FastAPI 在启动时会导入所有路由模块
   - `render.py` 路由模块导入了 `hifi_render_service`
   - `hifi_render_service.py` 的缩进错误导致模块导入失败
   - 模块导入失败导致 FastAPI 应用创建失败，服务无法启动

3. **如何避免类似问题**：
   - 使用代码格式化工具（如 `black`、`autopep8`）自动格式化代码
   - 使用 IDE 的自动缩进功能
   - 在提交代码前使用 `read_lints` 工具检查语法错误
   - 在修改代码后立即测试后端服务是否能正常启动
...
1. **检查后端服务状态**：
   ...
   # 检查启动日志
   tail -n 50 /tmp/backend_8081.log
   ```
...
3. **预期结果**：
   - 后端服务正常启动，日志显示 "Uvicorn running on http://0.0.0.0:8081"
   - 登录接口可以正常响应（即使密码错误，也应该返回 JSON 错误响应，而不是超时）
   - 前端登录请求不再卡死，可以正常显示错误信息

### 相关代码位置

- **后端渲染服务**: `server_py/app/services/hifi_render_service.py` (第 237-318 行)
- **后端路由**: `server_py/app/routes/render.py` (导入 `hifi_render_service`)
- **后端主入口**: `server_py/app/main.py` (导入路由模块)
- **前端登录 API**: `src/src/lib/api.ts` (第 321-345 行)
...
前端登录接口设置了 30 秒超时，但用户反馈登录请求可能因为后端启动失败而超时。

### 问题分析

前端登录接口的超时设置是合理的（30 秒），但需要确保：
1. 后端服务正常启动
2. 后端服务能够正常响应请求
3. 超时错误信息对用户友好
...
前端登录接口在 `src/src/lib/api.ts` 中设置了 30 秒超时：
...
1. **保持 30 秒超时**：对于登录接口，30 秒超时是合理的
2. **改进错误提示**：当超时发生时，提示用户检查后端服务是否正常运行
3. **添加重试机制**：可以考虑在超时后自动重试一次
