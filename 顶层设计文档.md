# 🧠 QuantaNova PhotoStyle Clone - 顶层设计文档 V4.1
...
| 组件         | 端口 / 说明                   |
|--------------|------------------------------|
| 前端 Vite    | **必须**运行在 `http://localhost:3001`（不允许使用 3000 或其他端口） |
| 后端 FastAPI | **必须**运行在 `http://localhost:8081` |
| MySQL (Docker)| localhost:3309               |
| 代理 (ClashX)| http://127.0.0.1:7890        |

**注意**：
- 前端端口必须在 `vite.config.ts` 中配置为 `3001`，与开发方案保持一致
- 后端 CORS 配置必须明确指定 `http://localhost:3001`，不能使用 `["*"]`（当 `allow_credentials=True` 时）
...
| 模块 | 文件 | 说明 |
|------|------|------|
| **入口文件** | `index.html` | Vite 入口文件（必需），包含 `<div id="root"></div>` 和 `<script type="module" src="/src/main.tsx"></script>` |
| **应用入口** | `src/main.tsx` | React 应用入口，将 App 组件挂载到 `#root` 元素 |
...
| API 封装 | `src/lib/api.ts` | `/api/analyze/part1`、`/part2`、`/api/simulate/style`、导出接口等 |
...
| Gemini 调度 | `src/services/geminiService.js` | 代理配置、重试、超时、响应解析、日志，使用 `@google/genai` 官方 SDK，默认模型 `gemini-3-pro-preview`（已迁移到 Gemini 3.0），风格模拟使用 `gemini-2.5-flash-image` |
...
> Gemini 配置：通过 `@google/genai` 的 `GoogleGenAI` 客户端调用官方 Gemini API，`ENV.GEMINI_MODEL` 默认使用 `gemini-3-pro-preview`（已迁移到 Gemini 3.0），可通过环境变量覆盖。支持 `thinking_level` 参数（`"high"` 或 `"low"`）控制推理深度。代理仍由 `ProxyAgent` 注入全局 `fetch`，重试与超时逻辑保持不变。详细迁移方案见 `Gemini3.0迁移方案.md`。
...
### 8.2 技术架构
- **模型使用**：
  - Part1 和 Part2 分析：使用 `gemini-3-pro-preview`（已迁移到 Gemini 3.0）
  - 风格模拟：使用 `gemini-2.5-flash-image`（保持不变，Gemini 3.0 不支持图片分割功能）
- **长上下文利用**：
  - Gemini 2.5 flash-image 支持 100 万个词元的上下文窗口
  - 可以记住之前的所有交互内容，包括 Part1 和 Part2 的输出
